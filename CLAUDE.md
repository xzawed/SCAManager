...
🔴 **정책 1 진화 (사이클 83 — 사이클 78~82 회고 Tier B-3 사용자 OK Q5)**: "전부다" / "모두 진행" / "모두 OK" 같은 **일괄 결정 발화 시** Claude 가 사용자에게 **검토 깊이 1줄 자가 보고 요청 의무**. 사이클 77 Q3 "전부다" (5 영역 모두) → 사이클 78~81 4 영역 100% 정합 진행 = Claude 권장 ★ 100% 일치 = 사용자 검토 깊이 미검증 위험 (사이클 76 회고 P0-3 알리바이 패턴 정합).
...
🔴 **정책 1 진화 강화 (사이클 84 — i18n 18 PR 회고 Tier B Q1 사용자 OK)** — **다중 PR 빠른 진행 신호 ≥ 10회 = 검토 깊이 자가 보고 요청 의무 default 추가**: 사이클 84 = 단일 작업일 18 PR 머지 (평균 머지 간격 20.6분/PR + 평균 LOC 600 = line-level 검토 사실상 불가) + 14회 연속 "머지 했습니다 다음 작업 진행" 신호. 사용자 발화 자체는 빠른 진행 (정책 1 진화 본 의무 미적용 발화 형식) but 누적 패턴 = "전부다" 일괄 결정과 동등 위험 영역. **default 적용**: 단일 작업일 사용자 빠른 진행 신호 (`머지 했습니다 다음 작업 진행` / `다음 진행해주세요` 등) ≥ 10회 도달 시점 = 검토 깊이 1줄 자가 보고 요청 의무. 사이클 76 회고 P0-3 알리바이 패턴 + 사이클 84 회고 관점 3 P0 정합.

**default 적용 시점 예시** (단일 작업일):
```
...
**예외 영역**: 사용자가 명시 신호 ("바로 진행" / "검토 OK 진행" / "오늘 종결까지 진행") 시 본 의무 적용 X (사용자 의도 명시 = 빠른 진행 default 정합).
...
**(1) 단일 작업일 5+1 디스패치 ≥ 5회 사용자 사전 확인 의무 신설**: 사이클 78~82 단일 작업일 = 6회 5+1 디스패치 (= 36 agent invocation = 누적 최고치 — 이전 사이클 비교 6배). 양호 ROI but 운영 비용 (Anthropic API token) + 사용자 검토 부담 누적 위험. **default**: 단일 작업일 5+1 디스패치 ≥ 5회 시점 = 사용자 사전 확인 의무 (Claude 자가 판단 X). 사용자 명시 신호 ("여러 에이전트 + 깊게" 등) 시 면제 OK. 정책 16 5번 원칙 (토큰 비용 효율) 페어.

🔴 **(1) 강화 (사이클 84 — i18n 18 PR 회고 Tier B Q2 사용자 OK) — "단일 작업일 dispatch 횟수" vs "누적 agent invocation" 구분 명시**: 사이클 84 = dispatch **2회** (기획안 5+1 + Phase 1 사전검토 3 에이전트) 만 발화 but 누적 agent invocation = **9회** (5+1 = 6 + 사전검토 3). dispatch 횟수 vs invocation 횟수 비대칭으로 정책 8 진화 (1) 임계 적용 영역 모호. **본 강화**:
...
**(2) 개별 PR cross-verify 효과 commit body 정량 명시 의무 (사이클 70~72 회고 P0-7 명시화)**: 5+1 cross-verify 진행 PR commit body §"cross-verify 결과" 섹션 의무 — false-positive 차단 N건 / 신규 발견 N건 / Tier A 정정 N건 정량 명시. 회고 보고서 부재 PR 도 ROI 추적 가능. 본 진화 = 정책 6 강화 (line:span 실측 의무) 페어 — Claude 추정 X 실측 의무.
...
## 바라는 점 (가장 중요한 N가지)
1. {사용자가 알기 어려운 정보 비대칭 항목}
2. {시범 운영 / 운영 사고 환류 요청}
3. {권장안 검토 깊이 / 결정 패턴 피드백}

## Claude 가 자성할 점
1. {본 사이클에서 Claude 가 잘못한 것 / 누락한 것}
2. {다음 사이클 개선 약속}

## 필요한 부분 (인프라 / 정책 / 정보)
1. {Claude 가 모르는 정보 — 사용자 트래픽 / 운영 데이터}
2. {다음 작업 우선순위 결정의 입력 부족}
...
- **자성 + 요청 균형** — Claude 가 자성만 하면 사용자 부담 ↑, 요청만 하면 책임 회피
...
🔴 **정책 9 완화 (사이클 83 진화 — 사이클 78~82 회고 P0 학습 — Q1 🅑 사용자 명시 결정)**: 회신 부재 default = **자율 판단 보고 명시 시 회신 의무 면제 OK** (Claude 자율 진입 가능). 사이클 78~82 = 6 사이클 누적 회신 0건 패턴 = 정책 9 본문이 alibi 화 위험 → 본 완화로 Claude 가 자율 판단 보고 (정책 3 강화 default — PR 본문 §자율 판단 보고 섹션) 명시 후 자율 진입 = **default 운영 정합 + 사용자 검토 부담 ↓**. **단, 다음 조건 미적용 영역**: (a) 운영 사고 차단 영역 (NEW-P0-N) — 정책 5 강화 페어 (NEW-P0-N 영역 = 매 사이클 진행 신호 회신 의무) (b) destructive 작업 (브랜치 삭제 / DROP / DELETE — 정책 7/12 페어) (c) architecture/UX/데이터 모델 결정 (메모리 `feedback-architecture-decision-pre-confirm.md` High tier) — 본 3 영역은 사용자 명시 회신 의무 보존.

**금지**:
- ❌ 회고 종합 보고 후 자유 발언 생략 (사용자가 명시 요청)
- ❌ "감사합니다" 같은 의례 발언만 (실질 피드백 X)
- ❌ 정책 9 완화 default 를 "회고 질문 작성 의무 면제" 로 해석 — 회고 질문 작성은 의무 + 회신 부재 시만 자율 판단 보고로 대체 OK
...
🔴 **정책 3 진화 강화 (사이클 83 — 사이클 78~82 회고 Tier B-2 사용자 OK Q5)** — **⚠️ 마커 정량 기준 의무**: 사이클 75 진화 = 이의 가능성 ≥ 중 = ⚠️ 마커 → Claude 자가 판단 영역 = 5 사이클 누적 ⚠️ 마커 commit body 0건 = 형식 알리바이 화. 본 진화 = 정량 기준 명시 (Claude 자가 판단 X):
- **자율 판단 보고 ≥ 5건** OR
...
→ 위 5 조건 중 1개 이상 충족 시 ⚠️ 마커 의무. **default 운영**: PR commit body 작성 시 자율 판단 보고 항목 카운트 + 위 4 영역 자가 검토 → ⚠️ 마커 적용 자율 판단 보고로 명시 (정책 9 완화 페어 — 자율 판단 보고 책임 명시).
...
gh api repos/xzawed/SCAManager/code-scanning/alerts \  # GitHub Code Scanning open alert 카운트 (정책 14)
  --jq '[.[] | select(.state=="open")] | length'        #   CI 부재 또는 인증 부재 시 사용자 GitHub Security 탭 직접 확인
ls ~/.claude/projects/-workspaces-SCAManager/memory/ \ # 신규 fixture/테스트/패턴 작성 전 메모리 grep — 사이클 64~67 P0 학습 (TDD Red / ASGI middleware / conftest setdefault / stale-blocker 트랩 차단)
  | grep -E "pytest-|test-|feedback-"                    #   해당 영역 메모리 본문 read 후 default 적용 의무
ls docs/reports/ | tail -1                              # 직전 회고 보고서 회신 회수 확인 (정책 9 강화 페어 — 사이클 83 진화)
  # 회고 §"회고 질문" 회신 부재 시 = 자율 판단 보고로 대체 OK (정책 9 완화 — 단, NEW-P0-N / destructive / High tier 영역은 회신 의무 보존)
                                                         #   현재 메모리 29건 (활성 27 + deprecated 2) — 사이클 75 카테고리 분류 (5 카테고리), 사이클 84 회고 Tier B Q4 +2 (i18n locale fallback + 메모리 카운터 패턴):
                                                         #   🌐 환경 (3): git-credential-helper-fix / nixpacks-gem-native-ext-trap / feedback-log-first-debugging
                                                         #   🧪 TDD/CI (7): test-env-setup / pytest-fixture-lazy-orm-import-trap / feedback-tdd-red-full-suite-validation /
                                                         #                  feedback-conftest-direct-env-set / feedback-ci-fixup-patch-coverage /
                                                         #                  feedback-testclient-lifespan-trap (사이클 78~81 회고 신설) / feedback-pr-push-direct-validation (사이클 82 PR 2 신설)
                                                         #   🤝 협업 (7): feedback-concise-docs-default / user-trust-model-and-tone /
                                                         #                feedback-think-before-code-edit (정책 15) / feedback-code-simplicity-default (정책 16) /
                                                         #                feedback-copilot-autofix-collaboration / feedback-copilot-autofix-noqa-trap (사이클 82 PR 2 신설) /
                                                         #                feedback-phase4-area-entry-pattern
                                                         #   📜 정책 페어 (4): feedback-architecture-decision-pre-confirm / feedback-stale-blocker-policy /
                                                         #                    feedback-ai-review-quality-protect / feedback-secret-scanning-history-rewrite
                                                         #   🛠️ 기술 패턴 (6): feedback-asgi-middleware-contextvars / feedback-defensive-coercion-mock-safety /
                                                         #                     feedback-wrapper-thin-deprecation-pattern / feedback-silent-fallback-streak-guard /
                                                         #                     feedback-i18n-locale-fallback-pattern (사이클 84 회고 신설) / feedback-memory-counter-pattern (사이클 84 회고 신설)
                                                         #   ⚰️ Deprecated (2): ~~phase3-rls-decision-pending~~ / ~~phase3-rls-runtime-activation-pending~~ (사이클 64/66 머지 완료)
                                                         #   상세 = ~/.claude/.../memory/MEMORY.md (사이클 84 회고 Tier B Q4 갱신)
...
- **CLAUDE.md 아키텍처 동기화 체크리스트**: `src/` 하위에 파일 추가 시 아래 항목을 순서대로 확인한다. 누락 시 다음 Phase 착수 전 반드시 보완한다. **전례 3건**: (1) Phase 11에서 6개 파일 추가 후 CLAUDE.md에 5개 누락 → 3-에이전트 감사에서 발견, PR #73. (2) 2026-05-01 UI 감사 cleanup PR-D1 — `_merge_attempt_states.py` (Phase 3 PR-B1 도입분) + `static/vendor/chart.umd.min.js` (UI 감사 Step C) 트리 누락 → 5-에이전트 정합성 감사에서 발견. (3) 2026-05-05 사이클 78~82 5+1 cross-verify — 사이클 79/80/82 신규 환경변수 4건 (`CLAUDE_INSIGHT_MODEL` / `SAAS_MULTITENANT_DISABLED` / `SAAS_ADMIN_EMAILS` / `SECURITY_AUTO_PROCESS_DISABLED`) `docs/reference/env-vars.md` 미등재 → 6차 cross-verify 발견. `env-vars.md` sync 의무 본 체크리스트에 신규 추가.
  ...
  | `docs/reference/env-vars.md` | **신규 환경변수 (`*_DISABLED` kill-switch / `SAAS_*` / 모델 분기 / DB 등) 추가 시 적정 섹션 (선택 / 내부 인증 / Observability / DB 등) 등재 의무** (사이클 82 5+1 cross-verify P0 학습 — 4건 누적 누락) |
