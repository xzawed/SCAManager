...
│   └── google.py               # /login, /auth/google, /auth/callback, /auth/logout (authlib OAuth2)
├── models/
...
│   └── user.py                 # User ORM (google_id, email, display_name)
├── webhook/
...
├── templates/
│   ├── base.html               # 공통 레이아웃
│   ├── login.html              # 로그인 페이지 ("Google로 로그인" 버튼)
...
tests/                          # 164개 단위 테스트 (Phase 1~8A + 버그 수정 + 점수 개선)
├── conftest.py
...
├── test_auth_google.py         # OAuth 라우트 단위 테스트 (Google API mock)
...
├── specs/2026-04-07-phase8a-auth-user-design.md
└── plans/ (phase1~8A 구현 계획)
```
...
- **Phase 8A 이후 로그인 필수**: UI 라우트(`/`, `/repos/*`)는 `require_login` Depends로 보호됨. 비로그인 시 `/login` 리다이렉트. `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`/`SESSION_SECRET` 미설정 시 OAuth 로그인 불가.
