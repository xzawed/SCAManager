...
- ✅ 진행 OK 영역: `review_code` 에 prompt caching 확대 / 모델 분기 (Haiku/Sonnet/Opus) / 동일 SHA 결과 재사용 / Insight narrative 호출 빈도 제한 / cache hit rate 모니터링 인프라
...
ls ~/.claude/projects/-workspaces-SCAManager/memory/ \ # 신규 fixture/테스트/패턴 작성 전 메모리 grep — 사이클 64~67 P0 학습 (TDD Red / ASGI middleware / conftest setdefault / stale-blocker 트랩 차단)
  | grep -E "pytest-|test-|feedback-"                    #   해당 영역 메모리 본문 read 후 default 적용 의무
                                                         #   현재 메모리 15건: pytest-fixture-lazy-orm-import-trap / test-env-setup
                                                         ...
                                                         #   feedback-ai-review-quality-protect (정책 16 5번째 원칙 — 명시 제외 영역)
                                                         #   feedback-secret-scanning-history-rewrite (사이클 71 학습 — git filter-branch 절차)
