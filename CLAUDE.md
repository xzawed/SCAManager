...
├── repositories/               # DB 접근 계층 9종 — repository_repo (find_by_full_name + Phase H 신규 find_by_full_name_with_owner opt-in joinedload), analysis_repo, analysis_feedback_repo, merge_attempt_repo, gate_decision_repo, repo_config_repo, user_repo, merge_retry_repo, security_alert_log_repo (사이클 73 F1)
└── worker/
...
5. **🔴 토큰 비용 효율** (사이클 72 추가 — 사용자 의도 정합) — 운영 토큰 사용량 ↓ + 분산 + caching 활용. 단, **AI 리뷰 품질 영향 영역 = 단순화 금지** (사용자 명시 제외 — 아래 명시 제외 영역). **caching 인프라 = 사이클 63 #218 도입** (`src/analyzer/io/ai_review.py:79` + `src/services/dashboard_service.py:546` 의 `build_cached_system_param` 호출 — 5분 ephemeral). **모니터링 인프라 = 사이클 72** (`src/shared/claude_metrics.py::get_cache_stats` + cache 비용 모델 정확화 + silent fallback WARNING).
...
ls ~/.claude/projects/-workspaces-SCAManager/memory/ \ # 신규 fixture/테스트/패턴 작성 전 메모리 grep — 사이클 64~67 P0 학습 (TDD Red / ASGI middleware / conftest setdefault / stale-blocker 트랩 차단)
  | grep -E "pytest-|test-|feedback-"                    #   해당 영역 메모리 본문 read 후 default 적용 의무
                                                         #   현재 메모리 18건 (deprecated 2 포함): pytest-fixture-lazy-orm-import-trap / test-env-setup
