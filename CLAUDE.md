...
- 🔴 **cross-verify 생략 PR 본문 대조 표 default (사이클 87 Tier B-3 — 사이클 86 회고 관점 2 P1)**: cross-verify 6차 생략 시 PR 본문 §"cross-verify 생략 사유" 에 사이클 69 정량 3 조건 대조 표 형식 default. 사이클 86 #324 commit body 누락 사례 학습 ("5 관점 일관 + 사용자 명시 검증 신호" 만 명시 — 정량 대조 부재). **default 표 형식**:
   ```markdown
   | 조건 | 충족 | 실측치 |
   |------|-----|--------|
   | (1) 1차 5 P0 합계 ≥ 8 | ✅/❌ | N건 |
   ...
   ```
   3 조건 모두 ✅ 시만 생략 OK. 1 조건이라도 ❌ 시 cross-verify 강제 (default).
...
**default 6-step**: `git checkout main && pull` → `checkout -b <type>/<scope>` → 작업+commit → `push -u origin` → **PR 직접 생성** (gh CLI > API + GITHUB_TOKEN > URL 폴백) → URL 보고.

🔴 **현재 SCAManager 환경**: gh CLI 부재 + GITHUB_TOKEN 401 → **옵션 🅒 (URL 폴백) default 운영**.
...
**메모리 인덱스 (13건 = 활성 13)**: 상세 = `~/.claude/.../memory/MEMORY.md`. 카테고리 2종:
- 📦 Project (5): score_bug_fix, db_migration, test_gap_analysis, settings_ux_redesign, poiesis_framework / 🤝 Feedback (8): test_patterns, railway_build, code_quality_patterns, documentation_principle, completion_workflow, pr_workflow, orm_integration_test, parallel_agent_branch
