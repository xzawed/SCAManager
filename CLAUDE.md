...
- **leaderboard_opt_in 컬럼 보존 의무 (그룹 60 Q3 결정)**: `leaderboard()` 함수 (그룹 60 Phase 1 PR 3 폐기) 와 별개로 `RepoConfig.leaderboard_opt_in` 컬럼 보존. 회귀 가드 = `tests/unit/services/test_analytics_service_deprecations.py:131` 명시. PRESETS 9개 필드 미포함 — 사용자 명시 옵트인 전용 (`/dashboard` Phase 3 사용자별 노트 가치 측정 입력).
