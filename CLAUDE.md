...
대시보드:
  → GET /                              (리포 현황 Web UI)
  → GET /dashboard?days={1|7|30|90}    (그룹 60 신설 — Phase 1+2: KPI 5 + 라인 차트 + 자주 이슈 + Auto-merge KPI + 실패 사유 + feedback CTA banner)
...
회고 배경: GitHub Code Scanning Alert #324 (`'break' or 'return' statement in finally`) + #325 (`Unused import`) 가 추적용 메타 Issue #213 / #214 로 발견. SCAManager 자체 정적분석 (pylint / flake8 / bandit) 은 통과 (위반 0건) 하지만 GitHub Code Scanning (CodeQL 또는 추가 도구) 이 별도 룰셋으로 감지하는 영역이 있음. **빌드 통과 + SCAManager lint 통과 ≠ Security 탭 0 alert**. 외부 가시성 부재 시 alert 누적 → 운영 책임 이전 위험.

**default 의무**:
- **작업 시작 전 30초 체크리스트** (CLAUDE.md L715~) 에 Code Scanning open alert 카운트 1줄 추가 (이미 적용)
- **매 사이클 종료 시** (또는 Phase 종료 시) — 정책 13 smoke check 와 페어로 Code Scanning open alert 검토
- **신규 alert 발견 시** — 분류 처리 (1회 의무):
...
모든 작업 착수 전 아래 네 가지를 순서대로 확인한다. 30초면 충분하다.
