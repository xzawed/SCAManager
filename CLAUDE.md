...
<!-- 권장 default 표 형식 예시 (상세: docs/policies/history.md#정책-1-진화)
```
| 옵션 | 장점 | 단점 | 위험 | 권장 시점 |
|------|------|------|------|----------|
...
🔴 **정책 1 진화 회귀 가드 (사이클 86 Q4 — 사용자 명시 결정)** — **자가 검증 의무**: Claude 가 일괄 결정 발화 직후 자가 보고 요청 누락 시 = **다음 응답에서 회복 의무** (자성 1줄 + 검토 깊이 사후 요청 + 자가 보고 1줄). 사이클 85 회고 P0 사례 = 본 회귀 가드 첫 적용. 회복 누락 시 = 다음 사이클 회고 §자성 명시 의무. 회복 패턴 예시: [docs/policies/history.md#정책-1-진화](docs/policies/history.md#정책-1-진화).

<!-- default 회복 패턴 예시 — 상세: docs/policies/history.md#정책-1-진화
```
...
<!-- cross-reference 4 정책 열거 상세 — docs/policies/history.md#정책-8-진화 참조
...
<!-- cross-verify 생략 PR 본문 대조 표 형식 (사이클 87 Tier B-3 — 사이클 86 회고 관점 2 P1)
사이클 86 #324 commit body 누락 사례 학습 ("5 관점 일관 + 사용자 명시 검증 신호" 만 명시 — 정량 대조 부재).
   ```markdown
   | 조건 | 충족 | 실측치 |
   |------|-----|--------|
   | (1) 1차 5 P0 합계 ≥ 8 | ✅/❌ | N건 |
   | (2) 관점 5종 모두 P0 ≥ 1 | ✅/❌ | 5/5 또는 N/5 |
   | (3) 사용자 빠른 진행 신호 명시 | ✅/❌ | "<발화 인용>" |
   ```
-->
...
<!-- 단일 관점 회고 + ROI 정량 상세 (사이클 75 진화)
단일 관점 회고: 관점 5종 중 1 영역만 분석하는 회고 (예: 사이클 70~74 회고 시 관점 2 단독 분석) = 5+1 패턴 자체 미적용 영역 — 정량 기준 (P0 ≥ 8 등) 검증 불가. Claude 직접 작성 default + cross-verify 생략 가능 (사용자 명시 신호 부재 시).
cross-verify ROI: 시간 비용 (~5분 병렬) vs 사고 회피 가치 (사용자 검토 부담 ↓ + 잘못된 결정 누적 차단). 사이클 75+ 운영 데이터 누적 시 기준 재검증 의무.
-->
...
<!-- 회고 질문 default 템플릿 (docs/policies/active.md 에서 관리)
```markdown
## 🔍 회고 질문 (사용자 회신 의무)
...
🔴 **사이클 94 사용자 정정 (2026-05-10)**: *"Codex에게 검토를 받으려면 PR을 올리기 전에 수행을 해야합니다."* → **Codex 검증 = PR push 전 의무**. push 후 의뢰 = 사후 의뢰 = mutual 의도 정합 X. 사이클 93 안티패턴 (PR #366/#368/#369/#370 = push 후 의뢰 명시 = 흐름 정합 X) 학습.
...
<!-- 환경 통합 상세 — Codespaces/자동화/hooks.json 경고 → docs/policies/active.md#정책-18 참조
- Codespaces 환경 default = 시나리오 A (사용자 별도 환경 Codex CLI 실행 + 채팅 회신)
- 사용자 회신 형식 3종: `Codex OK` / `Codex NG: <사유>` / Codex 출력 본문 채팅 붙여넣기
- 자동화 권장: PR template `[ ] Codex OK 회신 받음` 체크박스 (즉시) / Codex GitHub App + Actions workflow (사이클 94+ 옵션)
- ⚠️ `.codex/hooks.json` Windows hard-coded path → Linux Codespaces 환경 hook 0% 발화 (사용자 호스트 전용 — 자동화 영역 사전 인지 의무)
-->
...
- **완료 시 필수 5-step**: 작업이 완료되면 반드시 ① 커밋 → ② `git push` → ③ PR 생성(`gh pr create`) → ④ `docs/STATE.md` 수치 갱신 → ⑤ **CLAUDE.md 아키텍처 섹션 동기화** (신규 파일 추가·삭제·이름 변경 시 `src/` 트리와 `### 핵심 데이터 흐름` 내 언급 갱신) 를 순서대로 수행한다. 예외 없음.
- **README.md 배지 동기화**: 테스트 수·pylint·커버리지 수치가 바뀌면 `README.md` 21~25줄 배지도 함께 갱신한다. 수치 출처는 항상 `docs/STATE.md`.
- **CLAUDE.md 아키텍처 동기화 체크리스트**: `src/` 하위에 파일 추가 시 아래 항목을 순서대로 확인한다. 누락 시 다음 Phase 착수 전 반드시 보완한다. **전례 3건** (Phase 11 PR #73 / 2026-05-01 UI 감사 cleanup PR-D1 / 2026-05-05 사이클 78~82 5+1 cross-verify 환경변수 4건 누락) — 상세는 아래 주석 참조.

<!-- 전례 3건 상세:
(1) Phase 11에서 6개 파일 추가 후 CLAUDE.md에 5개 누락 → 3-에이전트 감사에서 발견, PR #73.
(2) 2026-05-01 UI 감사 cleanup PR-D1 — `_merge_attempt_states.py` (Phase 3 PR-B1 도입분) + `static/vendor/chart.umd.min.js` (UI 감사 Step C) 트리 누락 → 5-에이전트 정합성 감사에서 발견.
(3) 2026-05-05 사이클 78~82 5+1 cross-verify — 사이클 79/80/82 신규 환경변수 4건 (`CLAUDE_INSIGHT_MODEL` / `SAAS_MULTITENANT_DISABLED` / `SAAS_ADMIN_EMAILS` / `SECURITY_AUTO_PROCESS_DISABLED`) `docs/reference/env-vars.md` 미등재 → 6차 cross-verify 발견. `env-vars.md` sync 의무 본 체크리스트에 신규 추가.
-->
