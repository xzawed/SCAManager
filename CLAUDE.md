...
#### 정책 6 (보조): **다중 에이전트 디스패치 시 `line:span` 인용 의무 명시**
...
- **모든 sync PR commit body 에 `pytest tests/unit --collect-only -q` 실측 1줄 의무**:
  ```
  ## 검증 (사이클 N sync 실측)
  - 단위 = `pytest tests/unit --collect-only -q` → 2055 collected
...
**default 의무**:
- **작업 시작 전 30초 체크리스트** (CLAUDE.md L715~) 에 GitHub Security 탭 Code Scanning open alert 카운트 1줄 추가 (이미 적용)
...
5. **🔴 토큰 비용 효율** (사이클 72 추가 — 사용자 의도 정합) — 운영 토큰 사용량 ↓ + 분산 + caching 활용. 단, **AI 리뷰 품질 영향 영역 = 단순화 금지** (사용자 명시 제외 — 아래 명시 제외 영역). **caching 인프라 = 사이클 63 #218 도입** (`src/analyzer/io/ai_review.py:89` + `src/services/dashboard_service.py:571` 의 `build_cached_system_param` 호출 — 5분 ephemeral). **모니터링 인프라 = 사이클 72** (`src/shared/claude_metrics.py::get_cache_stats` + cache 비용 모델 정확화 + silent fallback WARNING). **활성화 + 호출 빈도 제한 = 사이클 74** (1024 토큰 패딩 + Haiku 모델 분기 + DB 캐싱 1h TTL).
...
ls ~/.claude/projects/-workspaces-SCAManager/memory/ \ # 신규 fixture/테스트/패턴 작성 전 메모리 grep — 사이클 64~67 P0 학습 (TDD Red / ASGI middleware / conftest setdefault / stale-blocker 트랩 차단)
  | grep -E "pytest-|test-|feedback-"                    #   해당 영역 메모리 본문 read 후 default 적용 의무
                                                         #   현재 메모리 21건 (deprecated 2 포함): pytest-fixture-lazy-orm-import-trap / test-env-setup
                                                         #   feedback-tdd-red-full-suite-validation / -architecture-decision-pre-confirm
                                                         #   feedback-asgi-middleware-contextvars / -conftest-direct-env-set / -stale-blocker-policy
                                                         #   feedback-log-first-debugging / -concise-docs-default / user-trust-model-and-tone
                                                         #   feedback-think-before-code-edit (정책 15) / -code-simplicity-default (정책 16)
                                                         #   feedback-ai-review-quality-protect (정책 16 5번째 원칙)
                                                         #   feedback-secret-scanning-history-rewrite (사이클 71 — git filter-branch)
                                                         #   feedback-defensive-coercion-mock-safety (사이클 72 — int 정규화)
                                                         #   feedback-wrapper-thin-deprecation-pattern (사이클 71 #238 — 4 조건)
                                                         #   feedback-silent-fallback-streak-guard (사이클 72 — N회 streak)
                                                         #   feedback-copilot-autofix-collaboration (사이클 73 #244 — pull --rebase + 메모리 grep)
                                                         #   feedback-ci-fixup-patch-coverage (사이클 73 #244 — 80% 사전 검증)
                                                         #   feedback-phase4-area-entry-pattern (사이클 73 #244 — 5+1 + Phase 분할)
                                                         #   git-credential-helper-fix / nixpacks-gem-native-ext-trap (환경 default)
                                                         #   ~~phase3-rls-decision-pending~~ / ~~phase3-rls-runtime-activation-pending~~ (deprecated)
