...
```bash
# 의존성 설치 (개발 환경 — pytest, playwright 포함)
...
3. 테스트 즉시 실행 가능 — `.env` 불필요 (conftest.py가 SQLite 인메모리 DB 사용)
   (`postCreateCommand`가 `requirements-dev.txt`를 자동 설치)
...
> 단위 테스트 전체(423개)는 API 키 없이 실행됩니다.
...
| `0010_pr_gate_three_options.py` | gate_mode→approve_mode rename + pr_review_comment·merge_threshold 신규 컬럼 추가 |
...
| `DB_SSLMODE` | PostgreSQL SSL 모드 (`require`, `verify-full` 등, 빈 문자열=미적용) | `require` | ❌ |
| `DB_FORCE_IPV4` | Railway IPv4 강제 연결 (온프레미스에서는 `false`) | `true` | ❌ |
...
| `DB_FAILOVER_PROBE_INTERVAL` | Primary DB 복구 확인 주기 (초, 기본 30) | `30` | ❌ |
...
├── database.py                 # SQLAlchemy engine, Base, FailoverSessionFactory — Primary 장애 시 Fallback DB 자동 전환, /health active_db 표시 (SQLite는 hostaddr 제외)
├── auth/
│   ├── __init__.py
...
│   ├── analysis_detail.html    # 분석 상세 — AI 리뷰·피드백·정적 분석 이슈 + 날짜·시간·소스·SHA title + 이전/다음 내비게이션 + 트렌드 차트(최근 30건)
...
tests/                          # 423개 단위 테스트 (Phase 1~8B + 점수 개선 + 알림 확장 + 분석 상세 + 커밋 메시지 + CLI + Hook + 보안 강화 + Webhook 재등록 + 분석 상세 표시 개선 + 이력 조회 강화 + nav 로그인/로그아웃 + 코드품질 강화 + 날짜 범위 필터 + 분석 상세 빈 result fallback + PR Gate 3-옵션 분리 + 테스트 커버리지 보강 + 점수 이력 차트 동기화 + DB Failover)
├── conftest.py
...
├── test_webhook_telegram.py    # Telegram callback endpoint + HMAC 보안 검증
...
├── test_main.py                # FastAPI app health/lifespan/라우터 등록 테스트
├── test_database.py            # _build_connect_args + engine pool 설정 테스트
├── test_failover.py            # FailoverSessionFactory — 단일엔진·자동전환·Primary복구·SSL중복방지 테스트 (26개)
└── test_github_helpers_models.py  # github_api_headers() + ChangedFile dataclass 테스트
...
│   └── onpremise-migration-guide.md      # Railway→온프레미스 마이그레이션 가이드
├── design/
...
.claude/
├── settings.json                       # Claude Code 훅 설정 (PreToolUse 보호 + PostToolUse 자동 테스트)
├── hooks/
│   └── check_edit_allowed.py           # 모바일 환경 보호 — 테스트 환경 없을 때 고위험 파일 수정 차단
├── agents/
│   ├── pipeline-reviewer.md            # 파이프라인 코드 리뷰 에이전트
│   └── test-writer.md                  # TDD 테스트 작성 에이전트
├── skills/
│   ├── lint.md                         # pylint + flake8 + bandit 실행
│   ├── test.md                         # pytest 실행
│   ├── phase-next.md                   # 다음 Phase 브레인스토밍
│   └── webhook-test.md                 # GitHub Webhook 로컬 테스트
└── plans/
...
- **_build_result_dict**: `src/worker/pipeline.py` 모듈 레벨 함수. pipeline과 hook.py 두 곳에서 Analysis.result dict를 생성할 때 사용. `score`·`grade` 필드 포함 — gate engine이 이를 기반으로 approve/merge 결정. 결과 구조 변경 시 이 함수만 수정하면 됨.
- **PR Gate 3-옵션 독립**: `pr_review_comment`·`approve_mode`·`auto_merge+merge_threshold` 완전 독립. `post_pr_comment`는 pipeline에서 제거되어 gate engine 내부(`run_gate_check`)에서 처리. `post_pr_comment_from_result(result: dict, ...)` 사용 — `AiReviewResult` 객체 불필요.
- **RepoConfig 필드 rename**: `gate_mode`→`approve_mode`, `auto_approve_threshold`→`approve_threshold`, `auto_reject_threshold`→`reject_threshold`. 기존 코드에서 구 필드명 사용 시 AttributeError 발생 — 반드시 신규 필드명 사용.
- **run_gate_check 시그니처 변경**: `(repo_name, pr_number, analysis_id, result, github_token, db)`. 기존 `telegram_bot_token`·`score_result` 파라미터 제거 — `settings.telegram_bot_token` 직접 사용, score는 `result["score"]`에서 추출. `pr_number=None`이면 모든 PR 액션 건너뜀.
- **analysis_detail 템플릿 context**: `src/ui/router.py`의 `analysis_detail` 라우트는 반드시 `current_user`를 template context에 포함해야 함. 누락 시 nav 사용자명·로그아웃 버튼 미표시. `analysis.result or {}` 패턴은 None → `{}` 변환으로 `{% if r %}` falsy 평가 → 모든 AI 섹션 숨김 버그 유발 — `{% else %}` 분기로 fallback 처리 필수.
- **analysis_detail trend_data**: `analysis_detail()` 라우트가 `trend_data`·`prev_id`·`next_id`를 template context에 추가로 전달. `trend_data`는 같은 리포 최근 30건 시간 오름차순 `{id, score, label}` 리스트. `trend_data|length > 1`일 때만 차트 렌더링(1건 이하 숨김). `analysis_detail.html`은 Chart.js CDN을 직접 로드(base.html 미포함 — 불필요한 페이지 로드 방지).
- **repo_detail 차트 동기화**: `buildChart(data)` 함수는 `data` 인자가 있으면 `_chartData`에 캐시, 없으면 캐시된 데이터 재사용. `applyFilters()` 호출마다 차트를 필터 결과와 동기화. `themechange` 이벤트는 `buildChart()` (인자 없음)으로 색상만 재빌드. Chart.js SSR 주입(`{{ chart_labels | tojson }}`) 방식은 제거됨 — 라우터의 `chart_labels`/`chart_scores` context 변수는 삭제 가능하나 하위 호환상 유지.
- **DB Failover 자동 전환**: `DATABASE_URL_FALLBACK`이 설정된 경우 `FailoverSessionFactory`가 Primary `OperationalError` 시 Fallback DB로 자동 전환. `_probe_primary_loop` daemon 스레드가 `DB_FAILOVER_PROBE_INTERVAL`초마다 Primary 복구를 확인하고 자동 복귀. Fallback 미설정(`DATABASE_URL_FALLBACK=""`)이면 단일 엔진 모드로 동작 (probe 스레드 없음, SELECT 1 오버헤드 없음).
...
| 날짜 범위 필터 | 분석 이력 날짜 프리셋 칩(오늘·금주·이번달·올해) + 직접 지정 date 입력 — 클라이언트 사이드 | ✅ 완료 (단위 297개) |
| 분석 상세 빈 result fallback | result=NULL/빈dict 시 분석 상세 페이지 콘텐츠 누락 버그 수정 + current_user nav 표시 수정 | ✅ 완료 (단위 300개) |
| PR Gate 3-옵션 분리 | gate_mode → Review Comment·Approve·Auto Merge 완전 독립 옵션 분리 + result dict 기반 PR 댓글 재설계 | ✅ 완료 (단위 321개) |
| 동일 점수 버그 수정 | SHA 중복체크 API 호출 이전으로 이동 + ai_review.status 추적 + ai_defaults_applied 플래그 + 대시보드 경고 배너 | ✅ 완료 |
| DB 연결 유연화 | Supabase→온프레미스 전환 지원 — _build_connect_args() + DB_SSLMODE/DB_FORCE_IPV4/pool 설정 추가 | ✅ 완료 |
| 테스트 커버리지 보강 | P0~P1 갭 해소 — main/health, helpers, models, gate 에러경로, auth OAuth, pipeline 디스패처 (단위 394개) | ✅ 완료 |
| 문서 재배치 | Claude Code 표준 구조 — plans→.claude/plans/, specs→docs/design/, guides/reports 분리 | ✅ 완료 |
