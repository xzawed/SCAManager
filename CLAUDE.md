...
**How to apply**: Edit/Write 도구 호출 직전 정책 15 사전 사고와 페어 — "이 변경이 가장 단순한 형태인가?" 자문 의무. CI lint (pylint R0911~R0917) 가 1차 가드, PR 본문 §"자율 판단 보고" 의 자가 리뷰 4 체크포인트가 2차 가드.
...
ls ~/.claude/projects/-workspaces-SCAManager/memory/ \ # 신규 fixture/테스트/패턴 작성 전 메모리 grep — 사이클 64~67 P0 학습 (TDD Red / ASGI middleware / conftest setdefault / stale-blocker 트랩 차단)
  | grep -E "pytest-|test-|feedback-"                    #   해당 영역 메모리 본문 read 후 default 적용 의무
                                                         #   현재 메모리 13건: pytest-fixture-lazy-orm-import-trap / test-env-setup
                                                         ...
                                                         #   feedback-think-before-code-edit (정책 15 페어) / -code-simplicity-default (정책 16 페어)
