...
_RLS_MATRIX: tuple[dict[str, str], ...] = (
    ...
    },
    {
        ...
        "pattern": "repo_id 간접 1-hop (repositories 페어, legacy NULL 호환)",
        "since": "0045",
        "status": "applied",
    },
)
...
def rls_coverage_summary(db: Session | None = None) -> dict[str, int | bool]:
    ...
    pg_class.relforcerowsecurity 실측 — _RLS_MATRIX 13 테이블 전부 FORCE 일 때만 True.
    ...
    pg_class.relforcerowsecurity — True only when all 13 _RLS_MATRIX tables are FORCEd.
