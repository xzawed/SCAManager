...
- 🔴 **AI 실패/절단 시 score/grade NULL-persist**: AI 리뷰 genuine 실패(`ai_review_status ∈ {api_error, parse_error}`) 또는 **diff 절단**(`result_dict["ai_review_truncated"]` — status=`success` 지만 부분-diff 인플레, C22) 이면 `Analysis.score/grade` 컬럼 NULL 저장 → analytics 집계(`func.avg`·leaderboard 의 `score IS NOT NULL` 필터)가 오염 자연 제외(쿼리 변경 0). `no_api_key`/`empty_diff`(의도적 미수행)는 점수 유지. pipeline `_save_and_gate` 단독(hook 은 `AiReviewResult.truncated` 미설정이라 절단 없음). `result` dict 의 마커(`ai_review_status`/`ai_review_truncated`)·`breakdown` 은 보존(진단/배너). 절단 auto-merge 차단은 #885 별도 가드.
