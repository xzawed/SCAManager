...
⚠️ owner-bypass 주의 (Task9 P1 #2): 본 마이그레이션은 RLS 를 ENABLE 만 한다 — FORCE ROW LEVEL
SECURITY 는 후속 0041(RLS Phase 3)이 11개 테이블 일괄 적용. 앱 연결 role 이 테이블 owner 면
PostgreSQL 이 owner 에 대해 RLS 를 기본 우회하므로, FORCE(0041) + 비-BYPASSRLS 앱 role 전환
(Phase 1/4 — docs/runbooks/rls-role-separation.md) + background worker 세션 분리(Phase 2, #847)가
모두 갖춰져야 2차 안전망이 실효화된다. admin /admin/rls-audit 페이지가 FORCE 적용 여부를
pg_class.relforcerowsecurity 실측으로 표기한다.
⚠️ owner-bypass caveat (#2): this migration only ENABLEs RLS — FORCE lands in 0041 (RLS Phase 3).
The 2nd safety layer takes effect only with FORCE (0041) + the non-BYPASSRLS app role switch
(Phases 1/4) + the Phase 2 worker-session split (#847).
...
# NOTE (#2): ENABLE-only here — FORCE lands in 0041 (RLS Phase 3).
