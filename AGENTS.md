...
→ [`CLAUDE.md §핵심 명령`](CLAUDE.md#핵심-명령) 참조 (설치·실행·테스트·lint 전체 명령표)
...
### 5. 코드 작업 전 사전 사고 (정책 15)
Edit/Write/MCP destructive 전 3 자문 의무 — (a) 목적 정합? (b) 영향 범위 인지? (c) 검증 방법 명확? 이해 부족 시 중단.

### 6. 문서 정리 안정성 우선 (정책 17)
문서 cleanup 시 안정성 > Anthropic 권장 규격. default rule + 진화 default 본문 보존 의무. 단계 분할 + 단계별 검증 의무.
...
→ [`docs/reference/env-vars.md`](docs/reference/env-vars.md) 참조 (전체 목록 + 설명)
...
| `pipeline.md` | `src/worker/pipeline.py`, `src/analyzer/**`, `src/scorer/**`, `src/webhook/**`, `src/gate/**` |
| `api.md` | `src/api/**`, `src/notifier/**`, `src/webhook/**`, `src/gate/**`, `src/main.py` |
| `security.md` | `src/auth/**`, `src/crypto.py`, `src/shared/log_safety.py`, `src/api/auth.py`, `src/webhook/validator.py`, `src/main.py` |
| `ui.md` | `src/templates/**`, `src/static/**`, `src/ui/**` |
| `i18n.md` | `src/i18n/**`, `src/middleware/locale.py`, `src/notifier/_language.py`, `src/analyzer/pure/review_guides/**` |
| `deploy.md` | `railway.toml`, `nixpacks.toml`, `requirements.txt`, `requirements-dev.txt`, `.env.example`, `alembic.ini`, `sonar-project.properties` |

---

## 완료 시 필수 6-step

① 커밋 → ② Claude/Codex 검증 의뢰 (push 전, 정책 18) → ③ `git push` → ④ PR 생성 (`gh pr create`) → ⑤ `docs/STATE.md` 수치 갱신 → ⑥ `docs/architecture.md` 동기화 (신규 파일 추가 시)

## 브랜치 명명 규칙

→ [`CLAUDE.md §브랜치-명명-규칙`](CLAUDE.md#브랜치-명명-규칙) 참조 (feat/fix/chore/docs 접두사 표)
