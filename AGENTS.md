...
> **왜 `불변식 4` 가 아닌가**: 리포 30여 곳이 `3-불변식` 을 참조하고, 이 문서가 이미
...
**2026-08-13 사용자 결정 (A+B 혼합 채택 — backlog R83).** 외부 적대 검증이 반례를 주면
...
- **실측 재발 (2026-08-12~13, 한 세션에 2회)**: `#1335` 가 Grok 반례(2000자 `X`)를 막고
  *"fail-open 을 닫았다"* 로 보고했으나, 회고가 **서사 100% 제거 + 지문·인용 채움**으로
  38건 전건 통과를 재현했다(backlog R81). 2026-08-15 사용자 결정 (b): 보존 축을
  내리고 도달성만 남겼다. 공동화된 아카이브는 설계상 green 이다. `#1340` 초판도
  같은 형태로 C3 BROKEN 판정을 받았다.
- **A 와 B 는 다른 것을 막는다** — A 는 결함을, B 는 거짓 보고를. 위 사고에서 나는
  **둘 다** 했다(가드는 뚫린 채 뒀고, 닫았다고 보고했다). 하나만 지키면 절반이 남는다.
