...
[![Tests](https://img.shields.io/badge/Tests-1102_passing-brightgreen?style=flat-square&logo=pytest&logoColor=white)](tests/)
...
Push 또는 PR 이벤트가 발생하면 **정적 분석**(pylint · flake8 · bandit · Semgrep · ESLint · ShellCheck)과 **Claude AI 리뷰**를 병렬로 실행하고, 100점 만점의 점수와 A~F 등급을 산출합니다.
...
| 분석 항목 | 도구 | 대상 |
|-----------|------|------|
| 코드 품질 | pylint + flake8 + Semgrep | `.py` (Python 전용 도구) + **34개+ 언어** (Semgrep) |
...
| **정적 분석** | pylint · flake8 · bandit (Python) + Semgrep (34개+) + ESLint (JS/TS) + ShellCheck (shell) |
