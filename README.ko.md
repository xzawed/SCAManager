...
[![Tests](https://img.shields.io/badge/Tests-1188_passing-brightgreen?style=flat-square&logo=pytest&logoColor=white)](tests/)
...
Push 또는 PR 이벤트가 발생하면 **정적 분석**(pylint · flake8 · bandit · Semgrep · ESLint · ShellCheck · cppcheck)과 **Claude AI 리뷰**를 병렬로 실행하고, 100점 만점의 점수와 A~F 등급을 산출합니다.
...
| 분석 항목 | 도구 | 대상 |
|-----------|------|------|
| 코드 품질 | pylint + flake8 + Semgrep + cppcheck | `.py` + C/C++ (cppcheck) + **35개+ 언어** (Semgrep) |
| 보안 취약점 | bandit + Semgrep | `.py` 파일 (테스트 제외) + 다언어 보안 룰 |
| JS/TS 품질 | ESLint (flat config) | `.js` `.mjs` `.ts` `.tsx` |
| Shell 품질 | ShellCheck | `.sh` `.bash` 등 shell 스크립트 |
| AI 리뷰 | Claude Haiku 4.5 | **50개 언어** 언어별 체크리스트 적용 |
...
| **정적 분석** | pylint · flake8 · bandit (Python) + Semgrep (23개+) + ESLint (JS/TS) + ShellCheck (shell) + cppcheck (C/C++) |
