...
| 2026-08-14 | [retrospective](2026-08-14-retrospective.md) | 정책 8 5+1 회고 (범위 #1340~#1346, 7 PR) — **179 에이전트**·14.2M 토큰·3h07m. findings **157** · **verdict_coverage 1.0** · 확정 143(P0 **6** · P1 53 · P2 84) · FP 기각 14 · 심각도 조정 51. 🔴 **P0 는 2 클래스로 수렴한다**: (a) 🔴 예산제의 분모가 4 glob 뿐 — *"100% 달성"* 을 선언한 그 커밋이 **계수 밖에 마커 30개를 함께 만들었다**(계수 안 92 vs 밖 505). 가장 값싼 우회로는 기호 치환이 아니라 **경로 이동**이다 (b) 회고 범위가 손입력이라 `#1347` 이 회고를 통째로 피했다 — `retrospective.mjs` 에 `retro_scope.py` 배선 **0건**, 2026-08-08 에 이어 **2회 연속**. 🔴 **세션 중 돌린 검증 2종이 P0-A 를 둘 다 놓쳤다** — 둘 다 *"이 가드가 이 규칙을 집행하는가"* 를 물었고 아무도 *"이 가드의 **분모가 어디까지인가**"* 를 묻지 않았다 |
