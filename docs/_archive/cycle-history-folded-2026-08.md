# 사이클 이력 원문 보존 (2026-08-13 압축)

> 🔴 **이 문서는 실행 대상이 아닙니다** — `docs/cycle-history.md` 에서 **1줄로 접은 섹션의 원문**입니다.
> 현재 이력은 `docs/cycle-history.md` 본문이며, 여기는 **그때 무슨 일이 있었는지**를 봅니다.

2026-08-13 압축에서 최근 **12개 세션**은 본문에 남기고 **143개**를 1줄로 접었다.
접은 것은 superseded 된 과거 사이클 서사이고, 각 줄은 여기 앵커로 되돌아온다.

---

<a id="회고-p1p2-이행--사용자-결정-3건--grok-상시-협업-2026-07-20-세션5-19-pr-11301149"></a>
...
<a id="pr-머지--railway-운영-실측--관측자-거짓말-사냥-2026-07-19-세션4"></a>
...
<a id="관측-복구-p0-3건--51-회고-2회--회고-조치-13-pr-2026-07-19"></a>
...
- 🔴 **#1110 P0 — Bash credential 덤프 차단 훅**: 정책 12 위반(`railway variables | grep` 이 값까지 매칭해 API 키 평문 출력)의 재발 방지책이 **산문뿐**이었다(인지 의존 산문 5회차). PreToolUse 훅 신설 + 차단 시 안전 관용구(`cut -d= -f1`) 안내. 오차단 0 실측.
...
- **B1(#1082)** `posttool_pytest_smoke.py` — 구 훅이 전체 5566 을 60s 타임아웃에 못 끝내고 삼켜 **false-green** → 편집 영역 스코프 스모크(전체 게이트=push-time 위임 명시).
- **B2(#1083)** `check_dead_code.py` — ADDED repo/service 공개 함수 src **AST 참조 0** 차단(docstring 제외·`# unwired-ok:` 예외) → #1060 find_orphaned 13 PR 생존 봉인.
- **C(#1085)** docs drift(repo 11→13·FastAPI 배지 0.139·cron 6 EN/KO parity·env-vars 3키·saas `_RLS_MATRIX` 13) · **D(#1084)** `owed-verification.md` 원장 · **P2#41(#1086)** route→service owner-filter parity 가드(#1074 봉인).
- **후속(#1088~#1092)**: owed #1058 email detail + 🔴 **자성** worktree 격리 규칙 확장(단일 background 에이전트 포함, #1088) · **P2#36** repo-integrity CI backstop(4 whole-repo 가드 push+PR·#1089) · **P2#18** `_begin_attempt` fail-safe(흔적 write 실패가 정상분석 abort 차단·pipeline-reviewer CONFIRMED·#1090) · **P2#17** merge_retry 4 종결경로 MergeAttempt 미러링(GC 전 이력보존·#1091) · **P2#43** retention DELETE PG round-trip 테스트(naive/aware·pending 절대보존·#1092). gone 브랜치 17 정리.
- 단위 5412→**5475**(+63) · 통합 154→**158** · E2E 122 불변. 🔴 owed 회신(#1058 SMTP·#1062 IDOR) · 보류(P2#16 claude_api_calls GC 보존기간 결정). [[project-retrospective-fixtracks-2026-07-18]]
- **후속 2(#1094)** 자초 CodeQL py/empty-except #547~#549 봉인 — 세션2 가드 3종이 `sys.stdout.reconfigure` 관용구를 **설명 주석 없이** 복사(4번째 사본 `check_retro_cadence.py:103` 만 주석 → 드리프트가 곧 alert 원인). 주석 통일 4곳(잠재 `translate_comments.py` 포함) + `test_empty_except_guard.py` 전역 AST 가드(탐지기 긍정/부정 통제 4건 동반·CodeQL parity). 🔴 부수 적발 = 카덴스 가드가 **Windows 에서 눈이 멀어 있었음**(subprocess `encoding=` 누락 → cp949 reader 크래시 → `r.stderr` 공백 → stderr 단언 무조건 통과). 단위 5475→**5480**(+5).
- **범위 한정 5+1 회고 (2026-07-19, wf_40082e43)** — 세션2 remediation 15 PR 메타-회고. 186 에이전트·3 라운드+완결성 표적 라운드·163 findings·**확정 147 (P0 2 · P1 55 · P2 90)**·반증 16·심각도 조정 67·verdict 커버리지 **1.0**. 🔴 핵심 결론 = **"가드의 가드" YES** — 세션2 신규 가드 4종 전부 결함(뮤테이션 실증: #1082 봉인대상 재주입해도 6/6 통과·#1080 완전 무력화해도 13/13 통과). 구조적 원인 = **"문서-only 시정" 3회차** + 자기참조 count-lock 3건(누락 방향 원리적 탐지 불가). 자초 CodeQL 재발 원인 반전 = 게이트 **부재**가 아니라 **note 임계값 미설정**(CodeQL 은 이미 PR 마다 SUCCESS 통과).
- **회고 P0 2건 조치(#1095)** — ① railway cron 5종 **전면 무음 실패**: API 키가 작은따옴표라 셸 미확장(리터럴 헤더→401) + `-f` 부재로 401 에도 exit 0(Railway 성공 기록). `-fsS` + 큰따옴표 + 독립 2조건 가드(셸 eval 실증). ② owed 원장 **write-only** → `check_owed_verification.py` 신설 + `.claude/settings.json` **SessionStart 훅 배선**(카덴스 카운터 동반 — stdout 이 Claude 컨텍스트 주입). 배선 가드는 산문 아닌 **실행 기전** 단언(뮤테이션 3종 탐지 실증). 단위 5480→**5518**(+38).
- **회고 P1 가드 탐지기 3건(#1096)** — 회고 "가드의 가드" 후속. ① `check_noqa_sideeffect` 코드 문자 클래스에 **공백 포함** → `F401  pylint` 를 `F401PYLINT` 로 뭉개 미탐지, flake8 은 실제 억제 → **실이력 90d0bd3 재검증 6건→9건(33% miss 해소)**. flake8 동형 분리 + **첫 비-코드 토큰 종료**(산문 속 'f401' 오탐 차단). ② CI 배선 가드 3종 `_git` **fail-OPEN → fail-CLOSED**(잘못된 base SHA·shallow clone 에서 "✅ 위반 없음 + exit 0" 이던 것을 exit 2 로 — PARITY GUARD 테스트가 3 사본 동등성 강제). ③ 🔴 **owed 검증 수단 정정** — Railway 실측 결과 *"cron 로그 관측"* 이 **실행 불가**(24h 로그 11줄·access log 0줄·cron 은 localhost 라 엣지 미경유·배포 항목 미기록) → **DB 부작용 관측**으로 대체. 🔴 **운영 실증**: 수정 전 cron 형태(리터럴 키) 재현 → **401**(`/health` 200 대비), 키는 설정돼 있음 = 셸 미확장 확정. 단위 5518→**5528**(+10).
- **회고 P1 근본 2건(#1097·#1098)** — ① 🔴 **CodeQL 원인 반전 실측**: alert instances API 로 자초 alert #547·#548·#549 가 **PR 시점에 이미 탐지**돼 있었음 확인(`refs/pull/1081·1082·1083/merge`). `note` 심각도라 체크가 SUCCESS 였을 뿐 → 근본은 '게이트 부재'가 아니라 **'임계값 미설정'**, 룰별 stdlib 가드 3종은 **이미 잡힌 것을 다시 잡는** 대응이었다("main 전체스캔만 노출"=증상, 기전 아님). `check_codeql_alerts.py` 게이트 신설(base 대비 신규만·note 포함·타임아웃=통과 금지·`github.sha` 머지커밋). 🔴 **생성 직후 #1096 에서 신규 alert 2건 실검출**(py/unnecessary-lambda) = **자초 CodeQL 4회차를 머지 전에 잡은 첫 사례**(이전 3회는 전부 머지 후 발견) → fix-up 교정. ② **dead-code 가드 이름 전역 매칭 봉인** — 무관 객체의 동명 속성 접근이 참조로 오집계돼 흔한 CRUD 명은 항상 통과했고 `find_orphaned` 는 **이름이 희귀해서** 잡혔을 뿐(성립 근거가 우연). 참조를 **정의 모듈에 한정**. 🔴 실증: `merge_retry_repo.list_pending` 이 `security_alert_log_repo.list_pending` 에 가려 구 구현 1건(배선 오판) → 신 구현 0건(미배선 정확). 부수 관측: 기존 공개 repo/service 109개 중 **9개가 한정참조 0**(diff 한정이라 미차단 — 정리 백로그). 단위 5528→**5551**(+23).
- 🔴🔴 **P0 — Railway cron 5종이 한 번도 실행된 적 없음 (#1099)**: `railway.toml` `[[deploy.cronJobs]]` 는 **Railway 스키마에 존재하지 않는 키**라 조용히 무시됐다(공식 문서 = 서비스당 **단일** `deploy.cronSchedule`, 배열 미지원). 실측 `cronSchedule=null`·`nextCronRunAt=null`. 결정적 증거 = 20:00 UTC 스윕 3.5h 경과 후에도 만료 캐시 **8건 잔존**(`purge_expired` 는 `expires_at < now` 삭제 → 실행됐다면 0). 🔴 **직전 #1095 의 따옴표·`-f` 수정은 실재 결함이었으나 명령 자체가 실행되지 않아 무의미**했다(진단 불완전 정정). ✅ 대체 = **인앱 스케줄러 `src/scheduler.py`**(lifespan 기동·운영 전용·의존성 0·stdlib) — JOBS 5종 등록, 배선을 `tests/unit/test_scheduler.py` 가 단언(저장소 밖 설정이 조용히 어긋나던 실패 모드를 코드로 이관). 🔴 **기존 가드가 신규 버그 적발**: 스케줄러가 bare `SessionLocal` 을 써서 세션 라우팅 인벤토리 가드가 fail → worker(BYPASSRLS) alias 로 교정(방치 시 RLS 가 `app.user_id` 미설정으로 평가돼 **job 이 0건만 보고 무동작**). `SCHEDULER_DISABLED` kill-switch · railway.toml 무효 키 재도입 차단 가드.
- 🔴 **앱 INFO 로그가 출시 이래 전부 소실 (#1100)** — `logging.basicConfig`/`dictConfig` 호출이 **한 곳도 없어** 모든 `logger.info` 가 핸들러 없는 root 로 갔다(Python last-resort = WARNING 이상만). Railway 로그에 uvicorn·alembic 만 보이던 이유 = 그 둘은 자체 핸들러 설정. 실측 영향: `retention sweep — purged …` 등 운영 관측 라인 전무 · owed 원장의 '로그 확인' 절차가 **물리적으로 불가능** · #1099 스케줄러 배포 검증 차단. `src/logging_config.py` 신설(stdout 핸들러·INFO·멱등) + `main.py` import 시점 배선.

<a id="세션2-회고--회고-fix-4-트랙-11-pr-2026-07-18"></a>
...
<a id="프리미엄-준비도-감사--wave-02-코드전용-8-pr-2026-07-18"></a>
...
<a id="grok-백로그-null-owner-idor--워커-내구성-6-pr-2026-07-17"></a>
...
<a id="설정-간소화--개요-count-up-봉인--회고-후속-2026-07-09"></a>
...
**⛔ 정책 18 폐기 (2026-07-10)**: 사용자가 Codex 유료 구독을 해지 → `@openai/codex` npm 패키지·Claude Code 플러그인 제거로 `codex` 실행 파일 부재 → Claude↔Codex mutual 검증 **수행 불가**. 저장소에서 제거: `AGENTS.md` · `.codex/`(agents 5·rules 8·hooks 2) · `.claude/skills/codex-verify.md` · PR 템플릿 §Codex 검증 의뢰 · 시작 체크리스트 turn-0 probe(#1049 에서 추가했던 것) · CLAUDE.md/active.md 정책 18 본문(history.md ⛔ 표기·상세는 git 이력 보존). 🔴 **존치**: 구 §2 의 `push 전 전체 pytest tests/unit 게이트`는 Codex 무관·유효 → 6-step ② 이관. 대체 = **Claude 단독 2-layer**(정책 8 5+1 관점 다양성 + `pipeline-reviewer`/opus whole-branch 적대 리뷰 — cross-cutting 결함의 유일 방어선이라 비용 절감 명목 생략 금지). 🔴 **보존**: 코드·문서의 "Codex mutual 적발/발견" 주석은 당시 사실 기록(출처 표기)이라 재작성 금지. cross-vendor 가치는 실증됨(2026-06-29 심층감사 P1 2건 = crypto 평문 fallback·STRICT_MIGRATION, Codex-only) → 대체 검증자 도입 시 근거 — 이 갭은 `docs/runbooks/integrity-audit.md` 에 **인지된 한계**로 명시. **후속 #1052·#1053**: (a) 잠긴 잔여 디렉터리(`.codex_tmp/`·`codex-pytest-*`)는 소유자가 샌드박스 로컬 계정(`CodexSandbox*`)이라 `icacls` 읽기조차 거부 → 🔴 `dirtc` 는 로컬 Administrators 멤버였으나 셸이 **UAC 필터링 토큰**이라 `takeown` 실패(`whoami /groups` 미표시·`net localgroup` 교차확인이 정답) → `Start-Process -Verb RunAs` 상승으로 실삭제 후 `.gitignore` 무해화 패턴 되돌림. (b) 완료된 계획·설계 문서에 `- [ ] Step N: Codex 검증 의뢰` 같은 **미완료 TODO 체크박스**가 남아 미래 세션이 할 일로 오인할 위험 → 16 문서(추적 9 + gitignore 로컬 7)에 ⛔ 배너 + 라인 마커 무력화. (c) 시스템 레벨: `~/.codex`(2,662 MB — 🔴 삭제 실패 원인은 권한이 아니라 **VS Code 확장이 띄운 `codex.exe` 2개의 sqlite 파일 잠금**) · 확장 `openai.chatgpt`(877 MB, 바이너리=`codex.exe`·남기면 `~/.codex` 재생성) · 로컬 계정 2 제거 = **~3.5 GB 회수**.
...
<a id="anthropic-비용-제어-3종-2026-07-08"></a>
...
<a id="51-회고-및-8-클러스터-fix-6-pr-2026-07-03"></a>
...
<a id="심층-감사-및-note-하드닝-2026-07-03"></a>
...
<a id="cross-vendor-감사-및-구조검토-2026-06-29"></a>
...
<a id="b-백로그-3건-구현-c-1-rls-live-검증-2026-06-29"></a>
...
<a id="전체-품질-감사-2라운드-다중에이전트-6-pr-codeql-봉인-2026-06-25"></a>
...
<a id="r13-평가--native-auto-merge-부적합retry-큐-영구-primary-2026-06-24"></a>
...
<a id="잔여작업--docs-drift-정정--회고-c10c11--c1-codeql-cascade-가드-2026-06-24"></a>
...
<a id="회고-follow-up--retrospectivemjs-dogfooding--하드닝-2026-06-23"></a>
...
<a id="repo-automation-pr-s--정책-흐름-스킬-3종-docs-sync--retrospective--codex-verify-2026-06-23"></a>
...
<a id="repo-automation-pr-w--워크플로우-loop-단일출처화--회고-워크플로우-신규-2026-06-23"></a>
...
<a id="회고-p2-백로그--envexample-무인증-footgun-제거--2nd-llm-검증자-활성화-runbook-2026-06-23"></a>
...
<a id="repo-automation-pr-h-신규-pre-commit-훅-3종-2026-06-23"></a>
...
<a id="회고-도구-개선-docs-repo-integrity-pre-commit-훅-2026-06-23"></a>
...
<a id="세션-다중-에이전트-회고-follow-up-p2-markdown-escape-4번째-채널-2026-06-23"></a>
...
<a id="감사-p2-하드닝--아웃바운드-markdown-인젝션-escape--단일출처-2건-2026-06-23"></a>
...
<a id="감사-보안-게이트-fail-open-봉인-3건-auth-fail-closed-static-crash-retry-sha-bound-2026-06-23"></a>
...
<a id="native-auto-merge-sha-atomicity-fail-closed-962-2026-06-23"></a>
...
<a id="정밀-감사69-에이전트-다차원-워크플로우--docs-drift-13건-정정-961-2026-06-23"></a>
...
<a id="ai-리뷰-점수-null-폐기-분리--입력-diff-절단-시-점수-보존-960-2026-06-22"></a>
...
<a id="sonarcloud-s6853-폼-라벨-7건-해소--edit-guard-hook-견고화--의존성-2026-06-22-후속"></a>
...
<a id="sonarcloud-잔여-critical-전부-해소-951954-2026-06-22"></a>
...
<a id="sonarcloud-blocker-및-고복잡도-critical-정리-948950-2026-06-22"></a>
...
<a id="sonarcloud-quality-gate-error-복구-폼-입력-20건-aria-label-2026-06-22"></a>
...
<a id="2nd-llm-머지-검증자-openai-호환-base_url-일반화-2026-06-19"></a>
...
<a id="개요-점수-0100-실제-repo개요-hx-boost-네비게이션-회귀-가드-e2e-추가-2026-06-19"></a>
...
<a id="정적-자산-immutable-캐시--배포-미전파-stale-사고-수정-2026-06-18"></a>
...
<a id="개요-점수-0100-count-up-고착--io-미발동-안전망--이중-init-dispose-회귀-p1-1c0a48375f942e-2026-06-18"></a>
...
<a id="repo_detail-점수추이-차트-미표시--i18n-스코프-격리-버그-933-2026-06-18"></a>
...
<a id="ai-리뷰-parse_error-근본-수정-931-2026-06-18"></a>
...
<a id="repos-모드-점수추이-차트-미표시-수정--921-후속-다중-차트-가드--전-모드-chartjs-로드-929-2026-06-17"></a>
...
<a id="품질-감사-p2-백로그-해소--코드-nit--문서-인용-2-pr-926927-2026-06-17"></a>
...
<a id="전체-문서코드-품질-감사-세션--9차원-다이나믹-워크플로우--p1-2건-해소-923924-2026-06-17"></a>
...
<a id="차트-hx-boost-async-로드-race-가드-live-머지--sync-921-2026-06-17"></a>
...
<a id="rls-phase-4-운영-전환-검증-완료-2026-06-16"></a>
...
<a id="잔여후속--회고-p2-마지막-테스트-하드닝-code-3test-2-919-2026-06-16"></a>
...
<a id="잔여후속--broad-docs-railway-ipv6-opt-in-정확화-918-2026-06-16"></a>
...
<a id="잔여후속--회고-p2-백로그-phase-4-transition-안전-하드닝-915916-2026-06-16"></a>
...
<a id="잔여후속--2026-06-16-세션-회고정책-8-51--option-a-follow-up-912914-2026-06-16"></a>
...
<a id="잔여후속--railway-pre-deploy--연결-invariants-docs--migration_database_url-906908-2026-06-16"></a>
...
<a id="마이그레이션-00390040-멱등화--운영-alembic-0038-고착-해소-904-2026-06-15"></a>
...
<a id="starlette-101-마이그레이션--dependabot-배치--902--897900-2026-06-15"></a>
...
<a id="회고-p2-백로그-해소--p2-ac22c12-3-pr-893895-2026-06-14"></a>
...
<a id="회고51-p1-follow-up--readmeko-배지888-정적-가드dbmd-u1-divergence-2026-06-14"></a>
...
<a id="정합성-감사-백로그-c12c22u1-머지--3-pr-884886-2026-06-13"></a>
...
<a id="잔여후속-세션--c1-save_gate_decision-dead-wrapper-제거-2026-06-13"></a>
...
<a id="잔여후속-세션--u2-effectsjs-hx-boost-애니메이션-재초기화-2026-06-13"></a>
...
<a id="정합성-감사-p2-백로그-처리--6-pr-874879-2026-06-12"></a>
...
<a id="전체-정합성-감사--보안correctness-p1-4-pr-868871-2026-06-12"></a>
...
<a id="잔여후속-세션--865-검증자-봉인-p1-1-반자동-parity-2026-06-12"></a>
...
<a id="잔여후속-세션--863-머지-검증자-봉인-p1-4-2026-06-12"></a>
...
<a id="잔여후속-세션--docs-정합회고p2-하드닝-860861-2026-06-11"></a>
...
<a id="2nd-llm-머지-검증자-도입-cross-vendor-ai-거버넌스-가드-2026-06-11"></a>
...
<a id="정합성-감사--deep-research-follow-up-2026-06-11--852856"></a>
...
<a id="사이클-166-적대-재검증-후속-2026-06-09--838841"></a>
...
<a id="잔여작업-라운드-2026-06-0910--843844-사용자-결정-c"></a>
...
<a id="잔여-정리-라운드-a옵션-2026-06-10"></a>
...
<a id="rls-2-phase-4-admin-대시보드-cross-tenant-보존--apiadminuiroutesadmin-hybrid-849-후속-2026-06-10"></a>
...
<a id="rls-2-phase-4-oauth-로그인-blocker-해소--auth_callback-worker-세션-전환-옵션-2-2026-06-10"></a>
...
<a id="rls-phase-1-운영--phase-3--0041-force--실측-가시화-2026-06-10"></a>
...
<a id="rls-phase-2--background-전용-worker-세션-분리-2026-06-10"></a>
...
## 부록 — 접기 전 목차 다이제스트 (136개)

> **왜 여기 있나 (backlog R89, 2026-08-14 사용자 확인 — 권장안 (c) 채택)**
>
> 2026-08-13 압축은 이 파일로 **본문을 옮겼지만**, `docs/cycle-history.md` 의 구 목차는
> 헤딩의 사본이 아니라 **손으로 쓴 사이클 다이제스트**였다 —
> `#1102~#1114 — alembic fileConfig 가 앱 로깅 런타임 파괴[#1100 무력화]·…` 같은 형태다.
> 헤딩에서 재생성할 수 없는 텍스트라 압축과 함께 **25,402자가 사라졌다**.
>
> 적대 검증(`wf_d4c837bb-603`)이 라인 집합 차집합으로 이 소실을 실측했고
> (136 라인 · 원본의 7.9%), 교차 검증이 *"본문 사실은 아카이브에 생존하므로 내용 소실이
> 아니라 **한 화면 탐색성** 손실"* 로 정정했다. 이 부록이 그 탐색성을 되돌린다.
>
> 🔴 **재생성이 아니라 원문이다** — 압축 직전 커밋(`c8db1995^`)의 목차 블록에서 그대로
> 뽑았다. 앵커는 이 파일에 실재하는 `<a id="…">` 와 대조해 붙였고,
> 대응이 없는 0건은 **링크로 만들지 않았다**(끊긴 링크를 만드느니 평문이 낫다).
...
- [사이클 110~116 (AI Insight 에러 추적·차트·다국어·감사 묶음, 2026-05-20~22)](#사이클-110111) — 상세: STATE.md 작업 이력 참조
