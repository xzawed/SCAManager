...
| # | 작업 | 종류 | 상태 | 사용자 소요 |
|---|------|------|------|-----------|
| 1 | [P4-Gate-2 Railway 실증](#1-p4-gate-2-railway-실증) | 🟢 실행 | **진행 필요** | 20~30분 |
| 2 | ~~Phase D.5~D.8 도구 확장~~ | — | ⏸️ **영구 보류 (2026-04-23)** — [결정 근거](../reports/2026-04-23-phase-e-service-pivot-decision.md) | — |
| 3 | ~~P3-후속 스캐폴딩~~ | — | ⏸️ **Phase E 완결 후 재검토** | — |
| 4 | ~~P5 pytest-cov devcontainer~~ | — | ⏸️ **현 CI 로 대체 (보류)** | — |

**진행할 것**: **1번 (P4-Gate-2)** 만 남았습니다. 완료 후 Phase E 단계는 Claude 가 단독 진행합니다.
...
## 2. ~~Phase D.5~D.8 정적분석 도구 확장 결정~~ ⏸️ 영구 보류

> **2026-04-23 결정**: Path A (서비스화) 선택에 따라 Phase D 확장 중단. 상세: [결정 문서](../reports/2026-04-23-phase-e-service-pivot-decision.md). 아래 내용은 역사 기록으로 보존.
...
## 3. ~~P3-후속 스캐폴딩 완성 (엔진 전환)~~ ⏸️ Phase E 완결 후 재검토

> **2026-04-23 결정**: 현 엔진이 기능상 정상 동작하므로 순수 리팩토링은 서비스화 우선순위에 밀림. Phase E.5 완결 후 재검토.
...
## 4. ~~P5 pytest-cov devcontainer 캐싱~~ ⏸️ 현 CI 로 대체 (보류)

> **2026-04-23 결정**: GitHub Actions CI 에서 커버리지 측정이 정상 동작 중 → 로컬 재현은 nice-to-have 수준. Phase E 완결 후 수요 있으면 재검토.
...
**P4-Gate-2 실증 하나만 남았습니다** (20~30분):

1. Railway CLI 로 컨테이너 접속 → `which rubocop`, `which golangci-lint` 확인
2. `xzawed/SCAManager-test-samples` 에 `unsafe_ruby.rb` 복사 → PR 제출
3. 동일 리포에 `unsafe_go.go` 복사 → PR 제출
4. SCAManager 대시보드에서 분석 결과 확인

상세: [P4-Gate-2 가이드](p4-gate-2-verification.md) 2~6단계

---

이후 Phase E 는 Claude 가 단독 진행합니다. 사용자 결정은 필요 없고, 각 Phase 완료 시 결과만 보고드립니다.
