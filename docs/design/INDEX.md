...
| 2026-04-27 | [tier3-native-automerge-design](2026-04-27-tier3-native-automerge-design.md) | Tier 3 — GitHub native `enablePullRequestAutoMerge` 통합 — **PR-A 완료 (#103)** / **PR-B1 완료 (#114, auto_merge_disabled webhook + MergeAttempt lifecycle)** / **PR-B2 완료 (#115, 이중 enable 가드)** / **PR-B3 폐기 보류 (R13 평가 2026-06-24)**: 운영 DB 실측 = native enable 0회(전 리포 "Allow auto-merge" OFF→422)·retry 큐가 운영 유일 머지 경로 → 즉시 폐기 = auto-merge 중단. 결정 = native 운영화(`allow_auto_merge` ON, SCAManager canary) 선행 → retry 큐 = 영구 fallback(하이브리드 공존이 최종 설계) |
