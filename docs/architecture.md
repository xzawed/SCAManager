...
├── models/                      # 11 ORM 모델 — repository (0039: user_id→users.id FK ondelete=SET NULL), analysis (0038: repo_id FK ondelete=CASCADE), analysis_feedback, repo_config (0036: disabled_tools JSON 컬럼), gate_decision (0034: analysis_id UNIQUE constraint), merge_attempt, merge_retry, security_alert_log, insight_narrative_cache (0031: repo_id FK; 0033: last_error_at/error_count/last_error_type), user (0040: github_id 인덱스명 정합 rename), issue_registration (0035: repo_id+issue_key UniqueConstraint + CASCADE FK; 0037: RLS policy — repo_id→repositories.user_id 1-hop, PG 전용)
├── webhook/
