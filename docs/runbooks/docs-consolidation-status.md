...
| PR | 묶음 | AR | 상태 | 산출 |
|----|------|----|------|------|
| **1** | B 실행 오인 어휘 전역화 | 9 | ✅ **완료** | `fix/plan-execution-cue-global` — 표지 11 + 가드 6축 |
| 2 | J 처분 지시 철회 + 원장 등재 | 6 | 🟡 다음 | 아래 §PR-2 참조 |
| 3 | E SESSION_SECRET 3분기 정정 | 6 | 🟡 다음 | 문서 3지점 |
...
**최소 실행 집합** = PR-2 · PR-3 (전부 독립, 선행 없음).
**순서 강제 1건** = PR-4 → PR-5. 역순이면 pylint 거짓값(10.00)이 기계로 봉인된다.
...
## PR-2 — 처분 지시 철회 + 원장 등재 (다음 착수)

계획서 §PR-2 참조. 🔴 **가드는 기각됐다** — 리포 내 실제 처분 지시가 0건이라 red 를 만들 수 없다.
산출물은 backlog 등재뿐이며, 그 결과 *"증명 없는 이동 금지"* 에 **집행자가 없다**(발행 규칙으로만 존속).

등재 대상 3건:
1. RRS 순위 발행 금지 — 채점 체계는 계측기로만 보존
2. `glossary.md` 처분 철회 근거 — 실행 시 `scripts/i18n_comments/translate_comments.py:48` 런타임 파손
3. 소비자 수집 4모드 미구현 (경로 조립 · 정규식/glob · bare-basename · 디렉토리 glob)
