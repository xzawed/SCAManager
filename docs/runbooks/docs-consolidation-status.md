...
| **4** | A pylint 진리값 (10.00→9.99) | 4 | ✅ **완료** (`#1331`) | 배지 5지점 + CI `--fail-under` 배지 파생 |
| **5** | C 게이트 예산 거짓 집행자 교체 | 3 | ✅ **완료** | `fix/gate-budget-real-enforcer` — 거짓 집행자 교체 + 가드 4 + 예산 상향 |
...
**다음 착수** = **PR-6**(가드 개수 산문 파생화 — backlog R86 과 같은 축).
🔴 **이 표는 두 번 낡았다** (PR-3 은 2026-08-14, PR-4 는 2026-08-14 발견). 착수 전 `git log --oneline --grep='문서감사 PR-'` 로 실제 머지분을 먼저 대조할 것 — 이 표를 믿고 재착수하면 순손실이다.
...
## PR-5 — 게이트 예산 거짓 집행자 교체 (✅ 완료, 2026-08-14)

**SPEC 4건 전부 이행**. 다만 계획서의 RED 예측 2건이 **실측과 달랐다** — 그 사이 문서가 자랐다.

| 축 | 계획서(2026-08-11) | 실측(2026-08-14) |
|---|---|---|
| `AGENTS.md` | 0.9006 (여유 1,193자) | **1.0003 — 이미 절단 중**(12,004/12,000) |
| `CLAUDE.md` | 0.4039 | 0.4563 |
| *"STATE 예산 4000 으로 되돌리면 red"* | RED 조건 | **green 이었다** — 아래 참조 |

🔴 **지문 단언은 예산 축을 못 잰다.** 계획서는 pylint 값이 offset ~11.2k 라는 전제로 그 RED
조건을 적었는데, 2026-08-05 STATE 재구조화로 종합 헤더가 맨 위로 올라와 지금은 **offset 1,915**
다. 예산을 4000 으로 낮춰도 값이 여전히 보인다. **지문이 우연히 앞쪽에 있으면 예산 축이
관측되지 않는다** — 그래서 슬라이스 **길이**로 재는 축을 따로 넣었다.

🔴 그 길이 축의 초판도 공허했다: 기대값을 `dict(_CONTEXT_SOURCES)[…]` 로 **피검사 모듈에서
유도**해, 예산을 낮추면 기대값도 함께 낮아져 green 이었다(A4 자기참조 공허화). 리터럴로 고정했다.

**사용자 결정**: `AGENTS.md` 예산 12,000 → **16,000 상향**(§6 결정 2 ★권장안 채택, 2026-08-14).

**뮤테이션 실증 4/4 red**: STATE 예산 4000 복원 · AGENTS 예산 12000 복원 · 예산 주석에 크기
리터럴 재도입 · README pylint 배지 값 변조.
