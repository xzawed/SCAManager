...
>
> 🔴 **잔여(동일 영역, 별도 결정 필요)**: `/admin/tenants`(`tenant_inventory`, `src/api/admin.py`)는 admin **세션**(require_saas_admin) RLS 라 admin 본인 행만 보여 타 테넌트 under-report (fail-safe — admin 이 더 적게 봄, 로그인/파이프라인 장애 아님). admin 크로스테넌트 가시성 복원은 **권한 상승(admin 이 전체 테넌트 행 조회) 결정**이라 별도 PR + 사용자 확인 필요(SaaS 멀티테넌트 모델 정의 페어).
