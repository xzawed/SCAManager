...
| **2** | `DATABASE_URL`=scamanager_app + `DATABASE_URL_WORKER`=scamanager_worker (Railway env) → 재배포 | 사용자 | 앱 기동 + `/health` 200 **+ 🔴 pre-deploy 마이그레이션 성공 로그 직접 확인** (아래 lifespan silent-fail 주의) |
