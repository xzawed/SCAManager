...
```
마이그레이션 실행은 별도 owner(`postgres`) 자격으로 (`alembic upgrade head`). 🔴 **단 현재 이를 보장하는 메커니즘이 없다** — `alembic/env.py:30` 이 `sqlalchemy.url` 을 무조건 `settings.database_url`(=위 `DATABASE_URL`=app)로 override 하므로, Phase 4 전환 시 pre-deploy(`railway.toml preDeployCommand`)·lifespan 마이그레이션이 app role(비-BYPASSRLS)로 돌아 `alembic_version` default-deny 에 막힌다. **Phase 4 전 `MIGRATION_DATABASE_URL`(owner) 분리 선행 필수**(§6 마이그레이션 credential 게이트).

### Phase 4 전환 실행 순서 (transition-day — 단계별 게이트)

> 🔬 **운영 현재 상태 실측 (2026-06-11, Supabase `qaoirpyhldlkeoyppfwq` read-only — 정책 12 SELECT 자율)**: Phase 1 role ✅ — `scamanager_app`(rolbypassrls=**false**)·`scamanager_worker`(rolbypassrls=**true**) 존재 + GRANT app/worker SELECT·INSERT 12/12 + policy 11/11. **단, alembic 버전 = `0038`**(로컬 head = `0041`) → **0039·0040·0041 미적용 = FORCE 미반영**(`pg_class.relforcerowsecurity=false` 11/11). `src/main.py` lifespan(`_run_migrations`)은 기동 시 `alembic upgrade head` 를 자동 실행한다. 🔴 **Phase 4 전 0번(배포) 선행 필수**.

| # | 작업 | 주체 | 게이트 (다음 단계 진입 조건) |
|---|------|------|------------------------------|
| **0** | 최신 main 운영 배포 → `main.py` 자동 `alembic upgrade head` 가 0039→0041 적용(FORCE on). 이 시점 아직 `postgres`(BYPASSRLS) 접속 = **차단 0**(안전). | 사용자 | `GET /admin/rls-audit` → `force_applied=True`. (`connection_bypasses_rls=True` + 경고 배너 = **정상** — URL 전환 전 BYPASSRLS 접속이라 기대값) |
| **1** | `scamanager_app`·`scamanager_worker` PW 교체(`ALTER ROLE ... PASSWORD '<신규시크릿>'`) — Phase 1 임시 PW 폐기 | 사용자 | 신규 PW 로 psql/SQL Editor 접속 성공 |
| **2** | `DATABASE_URL`=scamanager_app + `DATABASE_URL_WORKER`=scamanager_worker (Railway env) → 재배포 | 사용자 | 앱 기동 + `/health` 200 |
| **3** | 종단 검증 (§4 항목) | 사용자 | 로그인 smoke(기존+신규 GitHub) ✅ · `/admin/tenants` cross-tenant 가시 ✅ · background 파이프라인(webhook/cron) 차단 0 ✅ · pooler `SET LOCAL` 격리(deep-research P1) ✅ · `rls-audit` force_applied=True + connection_bypasses_rls=False + 경고 미표시 ✅ |
...
- [ ] 🔴 **마이그레이션 credential 게이트 — Phase 4 전 `MIGRATION_DATABASE_URL`(owner) 분리 (두 번째 벽, 정책 15 High tier)** — `alembic/env.py:30` 이 `sqlalchemy.url` 을 무조건 `settings.database_url`(=`DATABASE_URL`)로 override 한다. Phase 4 에서 `DATABASE_URL`=`scamanager_app`(비-BYPASSRLS)로 바꾸면 **pre-deploy `alembic upgrade head`(`railway.toml`) 와 lifespan `_run_migrations` 가 둘 다 app role 로 실행** → `alembic_version`(relrowsecurity=true·policy 0건 = default-deny)을 비-owner app role 이 못 읽어 마이그레이션 실패·배포 차단. 따라서 Phase 4 전에 마이그레이션 전용 owner credential(`MIGRATION_DATABASE_URL`)을 분리하고 pre-deploy/lifespan 이 이를 사용하도록 해야 한다(별도 코드 PR — DB/배포 스키마 사전 확인). 🔴 런타임 `DATABASE_URL`/`DATABASE_URL_WORKER` 를 마이그레이션 credential 로 수용 금지. (`pg_class` 실측: `alembic_version` relrowsecurity=true·policy 0건 — §Phase 1 운영 완료 노트.)
...
- [ ] 🔴 **0041 FORCE 운영 반영 확인 — `/admin/rls-audit` `force_applied=True`** (2026-06-11 실측: 운영 alembic `0038` < head `0041` → FORCE 미적용. Phase 4 전 최신 main 배포로 `main.py` 자동 마이그레이션 0039→0041 선행 의무. §Phase 4 전환 실행 순서 0번)
