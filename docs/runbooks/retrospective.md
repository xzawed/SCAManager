...
> 실행 엔진 = [`.claude/workflows/retrospective.mjs`](../../.claude/workflows/retrospective.mjs).
...
completeness critic + **cross-verify=finding 강제**(모든 finding 이 verdict 수신 → 단일 패스에서
일부만 검증되던 한계 해소). read-only 분석 — 코드/문서 수정은 호출자/사용자 책임.
...
> ```
>
> **왜**: 범위를 손으로 PR 구간으로 적으면 회고 착수 직전 머지분이 빠진다.
> 빠지는 것은 세션에서 **가장 마지막에 머지된 = 검증이 가장 덜 된** 산출물이다 —
> 정책이 막으려던 시나리오("가장 검증이 덜 된 코드가 회고를 피해간다")가 그 형태로 난다.
>
...
- 보고서 작성: `docs/reports/YYYY-MM-DD-retrospective.md` (ROI + verdict_coverage + 클러스터). 산출은 6-step 의무.

## verdict_coverage 해석
- `1.0` = 모든 finding 이 verdict 수신(목표). 단일 패스에서 일부만 검증되던 한계 해소 입증.
...
- 5 관점 × 다라운드(loop-until-dry, MAX_ROUNDS budget 시 5 / 미설정 3) + 전건 cross-verify = 다수 에이전트
  (다수 에이전트·다라운드). **실행 전 예상 비용 1줄 보고**(정책 16#5).
- 5+1(내부 self-verify, 관점 다양성) ↔ `pipeline-reviewer`/opus whole-branch 적대 리뷰(리뷰 계층 다양성) = **2-layer 독립** — 한쪽으로 다른 쪽 생략 금지. (구 외부 LLM mutual = 정책 18 폐기.)
...
## 알려진 한계
- ✅ **completeness 라운드 회복력**: try/catch 격리 — API 일시 오류 시 gap 라운드만 건너뛰고 본체 회고·Report 는 보존(이전엔 전체 소실 위험). 단 gap 라운드 자체는 미수행(best-effort).
- ✅ **evidence·citation_verified 환류**: 최종 confirmed 출력에 포함(보고서 추적성 복원, 이전엔 소실). adjusted_severity 도 명시 노출.
- ✅ **verdict_coverage < 1.0 자동 보강**: UNVERIFIED finding 만 1회 bounded 재검증(고동시성 StructuredOutput flake 해소) — 토큰 비용 최소(소수 한정). 1회 후에도 UNVERIFIED 면 보고서 unverified 표 노출(현행).
- ⚠️ SEVERITY_ADJUST 시 adjusted_severity 미수신이면 count 가 원 severity 로 graceful fallback — 스키마 강제(if/then) 미적용(검증기 스키마 위험 회피).
- 자유 발언·회고 질문·cross-verify 생략 정량 표는 워크플로우 출력 밖 = 스킬/Claude 절차 의존.
- 회귀 가드: `tests/unit/scripts/test_retrospective_resilience.py` (try/catch + evidence 출력).
