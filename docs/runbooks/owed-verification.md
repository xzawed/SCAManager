...
| **#1104** | 🔴 **`TELEGRAM_BOT_TOKEN` 로테이션** — 봇 토큰이 운영 로그에 **평문 기록**됨(httpx 가 요청 URL 전문을 INFO 로깅 · 실측 5건). `#1104` 는 **향후 유출만 차단**하며, **이미 로그에 남은 토큰은 코드로 회수 불가** | BotFather `/revoke` → 신규 토큰 발급 → Railway `TELEGRAM_BOT_TOKEN` 교체 → 알림 1건 유발해 정상 발송 확인. 배포 후 로그에서 `api.telegram.org/bot` 뒤가 `***` 로 마스킹되는지 함께 확인 | 5·14 | ⏳ |
| **#1106** | 🔴 **`INTERNAL_CRON_API_KEY` 로테이션** — **Claude 가 유발한 노출**: Railway 변수 조회 시 grep 패턴(`CRON`)이 **값까지 매칭**해 대화 기록에 평문 출력(정책 12 위반). 내부 cron 엔드포인트 전용이라 외부 노출 경로는 없으나, 기록에 남은 이상 교체가 맞다 | Railway 에서 신규 키 생성 → `INTERNAL_CRON_API_KEY` 변수 교체. 🔴 **재발 방지**: `railway variables --kv` 결과를 grep 하지 말 것 — `cut -d= -f1` 로 **이름만** 추출 후 필터 | 5·12 | ⏳ |
