...
| **#1295** | 🔴 **다음 세션에서 Claude 행동이 달라지지 않는가** — CLAUDE.md 를 424→196줄로 축소했다 | 🔴 **코드로 증명 불가**: 행동 규칙 생존 가드 30건이 *어휘의 삭제*는 잡지만 **규칙의 역전은 못 잡고**(실측), 무엇보다 *규칙을 읽고 지키는지*는 다음 세션의 실제 행동으로만 관측된다. Grok 이 1차에 **행동 규칙 8건 소실**을 잡았고 회고가 다시 3건을 잡았다 — **자기 검토만으로는 부족하다는 실증**이다. **확인 항목**: 규칙을 놓치거나 절차를 건너뛰면 알려 주실 것(해당 규칙을 본문으로 되돌린다) | 2·17 | ⏳ |
