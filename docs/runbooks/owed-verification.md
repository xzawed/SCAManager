...
| `INTERNAL_CRON_API_KEY` | ⚠️ Railway 에 설정됨·기능상 유효하나 **노출됨 → 로테이션 미결**(위 안전등급 `#1106`). "유효" 를 "안전" 으로 읽지 말 것 |
...
| **#1062** | NULL-owner 저장소 **쓰기 차단(IDOR)** 과잉 차단 여부 — 정상 소유자 흐름이 403 로 오차단되지 않는지 | ⏳ **사용자 회신 대기.** 🔴 **2026-07-19 자기 인증 철회** — Claude 가 이 행을 구조 논증으로 ✅ 처리했으나(#1129), 원장은 이 항목을 **"사용자만 할 수 있는 운영 검증"**(`docs/backlog.md:6`)으로 규정하고 **회신 전 상태 변경을 금지**한다(정책 5 NEW-P0-N · 정책 9 완화 미적용). 회고(2026-07-19, P0)가 적발해 원상 복구했다. **참고로 남기는 Claude 측 근거**(사용자 판단을 돕기 위함이지 검증 대체가 아니다): 차단 조건 전수 = `ui/_helpers.py:96`·`api/issue_registration.py:119`·`ui/routes/repo_insights.py:93` 3곳 모두 (a) `user_id is not None and user_id != current_user.id` → 404 (b) `require_write and user_id is None` → 403. 2026-07-19 DB 실측 = `repositories` 8건 중 NULL-owner **0건**·전부 `user_id=1`·`users` **1명**. 🔴 **이 근거의 한계**: 구조 불변식이 아니라 **가변 스냅샷**이다 — 사용자가 2명이 되거나 NULL-owner 저장소가 1건 생기면 두 조건 모두 즉시 도달 가능해지며, **그 변화를 감지하는 기계 트리거는 없다**. **필요한 것**: 운영 계정으로 본인 소유 저장소에서 설정 변경·피드백 등 7 쓰기 라우트가 200 인지 확인(약 15분) 후 OK/NG 회신 | 15 | ⏳ |
