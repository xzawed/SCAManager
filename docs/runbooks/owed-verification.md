...
| **#1289** | 🔴 **운영 `CLAUDE_REVIEW_MODEL` 값이 빈 문자열인가** — 빈 값이면 `#1289` 이전까지 **모든 AI 코드리뷰가 `api_error`** 였다 | 🔴 **코드로 증명 불가**: 로컬 `.env` 에서 이 결함을 실측·봉인했으나(빈 값 → `400 model: String should have at least 1 character`), **Railway 환경변수의 실제 값은 사용자만 볼 수 있다**. `#1289` 의 `field_validator` 가 앞으로는 빈 값을 기본값으로 되돌리므로 **미래는 안전**하고, 이 검증은 **과거 피해 범위 확인**용이다. **확인 항목**: (a) Railway 변수 `CLAUDE_REVIEW_MODEL` 이 비어 있었는가 (b) 비어 있었다면 대시보드에서 최근 분석의 `ai_review_status` 가 `api_error` 로 남아 있는지 — 그 기간 점수는 NULL-persist 라 집계에서 제외됐다(피해 = 리뷰 미제공, 점수 오염 아님). ⚠️ **변수 조회 시 `railway variables --kv` 를 grep 하지 말 것** — 값까지 매칭돼 평문 노출된다(`#1106` 학습), `cut -d= -f1` 로 이름만 | 13 | ⏳ |
| **#1289** | AI 리뷰 **품질 회귀 없음** — `output_config` json_schema 강제 후 리뷰 문장이 짧아지거나 어투가 바뀌지 않는가 | 🔴 **코드로 증명 불가**: 라이브 1회는 관측했으나(11필드 전건 채워짐 · 한국어 보존 · `status=success`), **반복 리뷰에서의 문장 품질·구체성은 사용자만 관측할 수 있다**. 정책 16 §명시 제외(AI 리뷰 품질 보존) 영역이라 특히 사용자 판정 몫이다. **확인 항목**: 실제 리포 push 로 리뷰 1~2건 받아 이전 리뷰와 비교. 나빠졌으면 `output_config` 한 줄 제거로 즉시 되돌릴 수 있다(스키마 정의는 남겨도 무해) | 2·13·16 | ⏳ |
