...
- ❌ **소유 금지 (authoring)** — Grok 은 정책·Issue 를 **저술하지 않는다**.
...
🔴 **거대 컨텍스트 덤프 금지** — *"과잉 컨텍스트는 리뷰어를 구조적 논평으로 민다."*
좁은 입력이 예의가 아니라 **경로 증명을 유지하는 수단**이다.

| 필드 | 필수 | 비고 |
|------|------|------|
| **CLAIM** | ✅ | 🔴 **평결이 아니라 공격받을 가설로** 제시 (사전 프레이밍이 다양성을 죽인다) |
...
Grok 은 MCP 가 없어 **Claude 의 배포 진실을 상속**한다 (*"당신의 거짓 green 이 곧 내 거짓 green"*).
