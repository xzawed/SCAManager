...
| 문서 · STATE · 정책 진화 | **Claude 전담** | 🔴 **호출 금지** (rule drift 위험) |
...
| **A2** | **신규 관측자에는 뮤테이션 없이 HOLDS 금지** — 새 seal(완전성 검사·가드·kill-switch)을 만들면 그 seal 자체를 최소 1회 깨뜨려 실패를 확인해야 HOLDS. 실행 주체는 Claude 든 Grok 이든 무방하나 **누군가는 실행**해야 한다 | 이번에 `assert X or True` 와 "folding 을 못 잡는 folding 테스트" 2건이 뮤테이션으로만 드러났다. 관측자를 만들며 관측자-거짓말을 재생산했다 |
