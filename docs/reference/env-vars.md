...
`DEFAULT_LOCALE` / `SUPPORTED_LOCALES` / `LOCALE_FALLBACK` / `I18N_TRANSLATIONS_DIR` / `I18N_DISABLED` 5개 환경변수로 Jinja2 i18n + Babel 백엔드 다국어 지원을 제어한다. LocaleMiddleware (Phase 1 PR-1b 영역) 가 요청 `Accept-Language` 헤더 → User.preferred_language → DEFAULT_LOCALE 순서 감지한다.
...
| `I18N_TRANSLATIONS_DIR` | Babel + Jinja2 i18n 번역 파일 위치 (상대 또는 절대 경로) | `src/i18n/translations` | 개발: `src/i18n/translations` / 운영: `/app/src/i18n/translations` |
