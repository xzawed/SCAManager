...
| **R33** | 🔴 결정 대기 | **`ANTHROPIC_API_KEY` 를 실제로 설정할지** + **INOPERATIVE 배너가 화면에 보이는지 미확인** | (a) 키를 설정하면 심의 게이트가 **처음으로 실제 작동**한다(문서 편집마다 Haiku 3회 ≈11.6k 입력 토큰). 안 하면 배너만 뜨고 게이트는 계속 무동작 — 둘 다 정당하나 **결정이 필요**하다. (b) 배너는 `_format_warn` 과 같은 **plain stdout** 이다. 리포에 `systemMessage` 관용구가 0건이라 검증 없이 새 출력 형식을 도입하지 않았다 — Claude Code 가 PreToolUse 의 비-JSON stdout 을 사용자에게 표시하는지는 **라이브 미확인**(Grok C2 caveat). 안 보인다면 배너는 theatre 다 |
