...
| **B8** | 🔴 **write-time fail-open 가드 탐지기** (2026-07-20 문서 재구성 Grok 최종 검증). 현재 가드 저술 3-불변식 중 **불변식 3(배선)만** 기계 강제(`test_guard_wiring_coverage`). **불변식 1(AST vs 산문)·2(실경로 뮤테이션-red)의 write-time 게이트 부재** — fail-open 가드(통과 조건이 bare substring)는 여전히 review/claim-review 에서만 잡힌다(rate-limiting step 미변, #1136/#1121/#1145 클래스). 처방: `scripts/check_*`·훅 신규/변경 저술 시 (a) 통과 조건이 bare `X in text` substring 인지 AST 로 탐지·차단 (b) 실경로 뮤테이션-red 증거 요구하는 blocking pre-commit/CI. 🔴 **로버스트 AST 탐지기는 신중 작업** — 성급하면 새 false-green(오탐으로 정상 가드 차단 or 미탐으로 무의미). 착수 시 A2 자기적용 의무 | Grok 이 6-Phase 재구성이 '볼륨만 옮기고 rate-limiting step 미변'이라 지목한 최고 레버리지 잔여 | 🟡 착수 가능(신중) |
