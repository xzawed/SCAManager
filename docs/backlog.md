...
| **R24** | 🟡 부분 이행 | **backlog 원장 자체의 정확성 6건** — R9 는 창에서 양 축 모두 해소됐는데 여전히 🟡 · ~~R2-b 의 반증 수단이 원리적으로 측정 불가(지정 API 가 영원히 404)~~ · 요약표 🔴 1건인데 실제 3건 · 회귀 가드가 원장 23행 중 5행만 본다 | 회고 P1 클러스터 6. "지금 뭐가 남았나" 의 SSOT 가 **완료된 일을 다시 시킨다**. 🔴 **본 항목 자신이 그 사례였다(세션13 정정)**: "R2-b 는 원리적으로 측정 불가" 는 **틀렸다** — 404 는 *보호가 없음*을 뜻하지 API 부재가 아니고, 보호를 적용하자 같은 API 가 **200 + `Repo integrity guards (stdlib backstop)`** 를 반환했다(실측). *현재 상태*를 *측정 가능성*으로 오독해 **해소 가능한 일감을 불가능으로 등재**한 것이다. 잔여 = R9 상태 · 요약표 카운트 · 가드 커버리지 3건 |
...
| **R33** | 🔴 결정 대기 | **`ANTHROPIC_API_KEY` 를 실제로 설정할지** + **INOPERATIVE 배너가 화면에 보이는지 미확인** | (a) 키를 설정하면 심의 게이트가 **처음으로 실제 작동**한다(문서 편집마다 Haiku 3회 ≈11.6k 입력 토큰). 안 하면 배너만 뜨고 게이트는 계속 무동작 — 둘 다 정당하나 **결정이 필요**하다. (b) 🔴 **부정 실측(2026-08-01)**: 배너 도입 후 CRITICAL 등급 문서(`CLAUDE.md` 2회 · `.claude/rules/guards.md` 1회)를 편집했으나 **Claude 의 도구 결과에 배너가 한 번도 나타나지 않았다**. 같은 세션에서 **PostToolUse** 훅 출력(`ide_diagnostics`)은 정상 주입됐고 SessionStart 훅 stdout 도 컨텍스트에 주입된다(런북 §6 이 실증) — 즉 **PreToolUse 의 비-JSON stdout 만 에이전트에게 도달하지 않는다**. 사용자 터미널 UI 에 보이는지는 여전히 미확인이나, **에이전트 채널에서는 theatre 로 확인됐다**. 🔴 **제안(사용자 결정 필요)**: 자격증명 부재 고지를 **SessionStart 훅으로 이관**한다 — 이 리포가 카덴스·owed·pre-commit 3종에 이미 쓰는 **실증된 채널**이다. ❌ **배제한 안**: PreToolUse 에 `permissionDecision: "allow"` + reason 을 내보내는 것 — 그 값이 사용자 권한 확인을 **건너뛸** 위험이 있어 검증 없이 도입 불가(안전 관련) |
> **상태 요약 — 이 표(현재 창) 18행 기준**: 🔴 결정 대기 **2**(R21·R33) · 🟡 착수 가능 **9** · ⏸️ 보류 **1**(R18) · ✅ 완료 **6**.
