...
**종합 수치**: 전체 **7243** 수집 (단위 **7072** + 통합 171) / E2E **121** (`#1291` 중복 제거 후; CI 120 통과 / 1 skip / 0 실패) / pylint **9.99/10** (`src/` — CI `lint-src` 가 `--fail-under` 를 README 배지에서 파생. `scripts/` 미게이트).

| 지표 | 값 | 비고 |
|------|-----|------|
| 전체 테스트 | **7243 수집** | `pytest tests/` — 단위 7072 + 통합 171 (현재). SSOT = 아래 §테스트 수 추적 이력 한 줄. |
...
- **현재** (7128→**7072** 단위; 통합 171 불변 = **7243** 수집, collect-only 실측 2026-08-16)
