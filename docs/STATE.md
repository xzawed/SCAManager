...
**종합 수치**: 전체 **7311** 수집 (단위 **7121** + 통합 190) / E2E **121** (`#1291` 기준) / pylint **9.99/10** (`src/`).

| 지표 | 값 | 재측정 |
|---|---|---|
| 전체 테스트 | **7311 수집** | `py -3 -m pytest --collect-only -q tests/unit` + `tests/integration` — 단위 7121 + 통합 190 (현재) |
...
- **현재** (7101→**7121** 단위; 통합 190 = **7311** 수집)
