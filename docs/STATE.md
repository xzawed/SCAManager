...
**종합 수치**: 전체 **8028** 수집 (단위 **7821** + 통합 207) / E2E **162** (`#1291` 기준) / pylint **9.99/10** (`src/`).

| 지표 | 값 | 재측정 |
|---|---|---|
| 전체 테스트 | **8028 수집** | `py -3 -m pytest --collect-only -q tests/unit` + `tests/integration` — 단위 7821 + 통합 207 (현재) |
| E2E 테스트 | **162개** | `make test-e2e` — = 162 collected (151 표준 + 11 perf). 대조 `scripts/check_e2e_scope.py` |
...
| 지원 언어 | AI 49 / 정적 27 | `src/analyzer/pure/language.py` · 등록 분석기 24종 |
...
- **현재** (7242→**7821** 단위; 통합 207 = **8028** 수집)
