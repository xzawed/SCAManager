...
**종합 수치**: 전체 **8170** 수집 (단위 **7963** + 통합 207) / E2E **332** (`#1291` 기준) / pylint **9.99/10** (`src/`).

| 지표 | 값 | 재측정 |
|---|---|---|
| 전체 테스트 | **8170 수집** | `py -3 -m pytest --collect-only -q tests/unit` + `tests/integration` — 단위 7963 + 통합 207 (현재) |
| E2E 테스트 | **332개** | `make test-e2e` — = 332 collected (321 표준 + 11 perf). 대조 `scripts/check_e2e_scope.py` |
...
- **현재** (7242→**7963** 단위; 통합 207 = **8170** 수집)
