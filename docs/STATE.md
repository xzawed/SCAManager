...
**종합 수치**: 전체 **8150** 수집 (단위 **7943** + 통합 207) / E2E **136** (`#1291` 기준) / pylint **9.99/10** (`src/`).

| 지표 | 값 | 재측정 |
|---|---|---|
| 전체 테스트 | **8150 수집** | `py -3 -m pytest --collect-only -q tests/unit` + `tests/integration` — 단위 7943 + 통합 207 (현재) |
...
- **현재** (7242→**7943** 단위; 통합 207 = **8150** 수집)
