...
**종합 수치**: 전체 **7455** 수집 (단위 **7284** + 통합 171) / E2E **121** (`#1291` 이 중복 1건 제거) — CI 실측 **120 통과 / 1 skip / 0 실패**(`#1294`, 2026-08-06). 🔴 배선(`#1288`) 이래 **처음 전건 초록**이다: 스위트-앱 drift 30건(`#1291`) · CSP 가 자기 폰트를 차단하던 앱 결함 · CI 가 CSS 빌드 없이 돌던 설정 결함(`#1294`)을 순서대로 해소. backlog R52 / pylint **9.99/10** (src/ — 🔴 **CI `lint-src` job 이 `--fail-under` 를 **README 배지에서 파생**해 게이트**한다(2026-08-12 문서감사 PR-4 — 이전 리터럴 9.90 은 9.99 도 10.00 주장도 전부 통과시켰다). scripts/ 는 미게이트).

| 지표 | 값 | 비고 |
|------|-----|------|
| 전체 테스트 | **7455 수집** *(헤더 = 최신값, 이 셀 = pytest 누적 추적)* | `pytest tests/` — 단위 7284 + 통합 171 (현재). 누적 이력은 **아래 §테스트 수 추적 이력** 참조 (단위 baseline 4718). |
