...
**종합 수치**: 전체 **7461** 수집 (단위 **7290** + 통합 171) / E2E **121** (`#1291` 이 중복 1건 제거) — CI 실측 **120 통과 / 1 skip / 0 실패**(`#1294`, 2026-08-06). 🔴 배선(`#1288`) 이래 **처음 전건 초록**이다: 스위트-앱 drift 30건(`#1291`) · CSP 가 자기 폰트를 차단하던 앱 결함 · CI 가 CSS 빌드 없이 돌던 설정 결함(`#1294`)을 순서대로 해소. backlog R52 / pylint **9.99/10** (src/ — 🔴 **CI `lint-src` job 이 `--fail-under` 를 **README 배지에서 파생**해 게이트**한다(2026-08-12 문서감사 PR-4 — 이전 리터럴 9.90 은 9.99 도 10.00 주장도 전부 통과시켰다). scripts/ 는 미게이트).

| 지표 | 값 | 비고 |
|------|-----|------|
| 전체 테스트 | **7461 수집** *(헤더 = 최신값, 이 셀 = pytest 누적 추적)* | `pytest tests/` — 단위 7290 + 통합 171 (현재). 누적 이력은 **아래 §테스트 수 추적 이력** 참조 (단위 baseline 4718). |
...
- **세션19 27차 — R80 옵션 A: consistency 심의자에게 6-step ⑤ 파생 관계를 알린다 (Grok 저술) +4** (7286→**7290** 단위 — 프롬프트 계약 핀 4건(파생 계약 · 범위 울타리 · R37 생존 · frontmatter 파싱); 통합 171 불변 = **7461** 수집, collect-only 실측 2026-08-15). 🔴 **2026-08-15 사용자 결정 = A** (`#1363` 옵션 표). B(impact 강등 경로) · C(원 처방 src 발췌) 기각 — C 는 `#1357` 9에이전트 조사가 반증했다. **결함**: 6-step ⑤ 는 손 편집을 원장 꼬리 한 줄로 제한하고 나머지 4지점을 `check_docs_sync --fix` 가 파생시키므로, 그 절차 중간 diff 는 **꼬리=새 값 · 파생=옛 값**이 정상인데 consistency 가 그것을 «수치 불일치» 로 읽어 `block` 했다 — **게이트가 의무 절차를 막았다**(2026-08-11 실측, `#1357` 라이브 기록). **수정**: `.claude/agents/doc-consistency-reviewer.md` 에 파생 계약 + ❌/✅ 쌍 + **범위 울타리 3종**(꼬리 자체 내부모순 · 비-파생 문서의 제3의 수 · 파생끼리 불일치 = 여전히 block). 울타리 없이 면제만 두면 리뷰어가 STATE 수치 일반에 눈먼다. **가드**: `_read_agent_prompt` 로 로드해 경로 하드코딩을 피했고(로더 리네임·frontmatter 회귀도 red), **헤딩만 남기고 본문을 비워도 red** 임을 뮤테이션으로 확인했다 — 헤딩 존재 검사는 공허 섹션을 통과시킨다(traps B5 회피). R37 절 삭제도 red. ⚠️ **A 는 R80 을 전부 닫지 않는다** — «심의 컨텍스트가 `src/**` 를 못 읽는다» 축은 그대로다. A 는 그것이 **실 차단 원인이 아니었다**는 `#1357` 반증 위에 선다. 확률적 리뷰어의 프롬프트 수정이라 **봉인이 아니다**. 🔴 **이 세션에 게이트가 R96 을 재현했다** — backlog 편집에서 consistency 가 사유란에 *«R80 선택이 정책 19와 모순»* 이라 적으면서 판정은 `[OK]` 를 냈다(라벨↔사유 분리). 🔴 **26차 항목 정정**: 위 항목의 *«고치지 않고 보고만 한 것 4건»* 은 인계문서 §4 기준 **5건**(공허 4 + C7 등재 1)이다. 같은 오기가 `docs/README.md:67` 색인에도 있었고 `#1363` 이 그쪽을 고쳤다. 원장은 append-only 라 위 항목을 재작성하지 않고 여기 정정만 단다. (#1363 결정 · 이 PR 이행)
