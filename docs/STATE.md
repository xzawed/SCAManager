...
**종합 수치**: 전체 **8173** 수집 (단위 **7966** + 통합 207) / E2E **352** (`#1291` 기준) / pylint **9.99/10** (`src/`).

| 지표 | 값 | 재측정 |
|---|---|---|
| 전체 테스트 | **8173 수집** | `py -3 -m pytest --collect-only -q tests/unit` + `tests/integration` — 단위 7966 + 통합 207 (현재) |
| E2E 테스트 | **352개** | `make test-e2e` — = 352 collected (341 표준 + 11 perf). 대조 `scripts/check_e2e_scope.py` |
