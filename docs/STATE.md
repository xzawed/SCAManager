...
**종합 수치**: 전체 **8027** 수집 (단위 **7832** + 통합 195) / E2E **121** (`#1291` 기준) / pylint **9.99/10** (`src/`).

| 지표 | 값 | 재측정 |
|---|---|---|
| 전체 테스트 | **8027 수집** | `py -3 -m pytest --collect-only -q tests/unit` + `tests/integration` — 단위 7832 + 통합 195 (현재) |
...
- **현재** (7242→**7832** 단위; 통합 195 = **8027** 수집)
