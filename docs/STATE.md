...
**종합 수치**: 전체 **7180** 수집 (단위 **6998** + 통합 182) / E2E **121** (`#1291` 기준) / pylint **9.99/10** (`src/`).

| 지표 | 값 | 재측정 |
|---|---|---|
| 전체 테스트 | **7180 수집** | `py -3 -m pytest --collect-only -q tests/unit` + `tests/integration` — 단위 6998 + 통합 182 (현재) |
...
- **현재** (6996→**6998** 단위; 통합 182 = **7180** 수집)
