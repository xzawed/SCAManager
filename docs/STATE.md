...
## 현재 수치 (2026-05-10 기준 — **사이클 94 종결 + UI 일러스트 Step 2-B (#375) + Tailwind v4 빌드 파이프라인 (#376~#379)**: 정책 18 신설 (Claude ↔ Codex mutual) + 첫 NG → 코드 수정 → OK 첫 운영 사례 (#372) + DALL-E 3 일러스트 5장 + 5페이지 마크업 (#375)): 130+ PR #188~#379 — 누적 정책 본문 19건 (정책 18 신설) + 메모리 30건 (활성 28 + deprecated 2 — 누적 검증 정기 sync PR 영역 보류). **사이클 94 (#372)**: chore/cycle-93-residual-tasks (529c0a5) cherry-pick → Codex IDE Extension review NG → `_reset_repo_config()` 헬퍼 신설 → OK → push (mutual 첫 운영 검증). P1-3a-2 (simple 모드) = #372 로 해소 (test_save_success ordering 트랩 차단). P1-3a-1 (944px grid) = UX 결정 영역 보류 (다수 자식 grid 도입 시 재활성화). **#376~#379**: Tailwind v4 Hybrid 빌드 파이프라인 신설 + nixpacks `pip: command not found` (exit 127) 수정 — `[phases.install]` 제거로 Python venv default 복원 + `npm ci && npm run build` railway.toml buildCommand 이전. Railway 배포 `03b10301` SUCCESS 확인. 사이클 92~93 누적 (#362~#371) + 메모리 카운터 정확 검증 = 정기 sync PR 영역 (정책 17 5번째 default 트리거 시점 — ≥ 5 사이클 경과 또는 누적 ≥ 50 PR). 단위 2682 / 통합 129 / E2E 96 / pylint **9.94/10**

| 지표 | 값 | 비고 |
|------|-----|------|
| 단위 테스트 | **2682개** | pytest 9.0.3 — 사이클 73~75 +67 + 사이클 78~82 +114 (2122→2236) + 사이클 84 i18n 18 PR +473 누적 + **사이클 85 Sentry 완전 제거 -40** (test_observability 9 + test_observability_before_send 14 + test_sentry_scrubbing 17) + **#375 일러스트 회귀 가드 +13**. **= 2682 collected / 2681 passed / 1 skipped / 0 failed** |
...
### 사이클 68 — 4 사이클 종결 회고 + 정책 진화 + 종료 sync (2026-05-04 · PR #232 ~ #234)
...
| 영역 | 핵심 |
|------|------|
| **Tier B-1** Makefile `lint-strict` target | `pylint --fail-under=9.90 src/` (pylint 단독 — flake8/bandit 은 기존 `make lint` 에 보존, 정책 16 단순화 default). pylint drift 자동 감지 회귀 가드. baseline 9.94 보수적 floor. CI workflow 통합 = 사용자 명시 의무 영역 |
| **Tier B-2** dependabot.yml `groups` 분리 | pip 영역 production-deps (minor/patch) + development-deps (minor/patch) 분리 + github-actions 영역 actions-minor-patch 묶음. 사이클 86 8 PR 사례 (#325-#332) 학습 = major bump + security update = group 외 개별 PR (변경 로그 검토 의무 페어) |
| **Tier B-3** cross-verify 생략 PR 본문 대조 표 default | CLAUDE.md L257 정책 8 본문 진화 — cross-verify 6차 생략 시 PR 본문 §"cross-verify 생략 사유" 에 사이클 69 정량 3 조건 대조 표 형식 default. 사이클 86 #324 commit body 누락 사례 학습 |

**자율 판단 보고 (정책 3 ⚠️ 마커)**:
- ⚠️ Tier B-1 `lint-strict` = 새 target 추가 (기존 `lint` 보존). flake8 fail (E116/F401/E501 9건) 으로 1차 fail → pylint 단독 회귀 가드로 분리 (정책 16 단순화). CI workflow 통합 = 사용자 명시 의무
- ⚠️ Tier B-2 dependabot groups = 사이클 88+ 다음 dependabot 주기 (월요일 09:00 KST) 검증 의무
- ⚠️ Tier B-3 정책 8 본문 진화 = 사이클 88+ 첫 적용 (본 사이클은 신설 자체)

---

### 사이클 86 — CI submit-pypi 영구 실패 대응 + Dependabot 자동 8 PR + pylint drift 회복 + 5+1 회고 (2026-05-06 · #321 + #322 + #324 + #325-#332 + #334 + #335 + #336)
...
- ⚠️ **pylint 잔여 36건 사이클 87+ 점진** — 1 PR 일괄 fix 시 변경 범위 ↑ + 검증 부담 ↑. 정책 16 단순화 default + 정책 7 강화 응집 단위 페어
- ⚠️ **본 sync PR = 사이클 86 종결 명시** + 사용자 manual 의무 영역 (Step 1+2+3) 명시 — 정책 12 destructive 영역 사용자 명시 의무 보존

**사용자 manual 의무 영역 (정책 13 페어)**:
- Settings > Code security > Dependency graph > Automatic Dependency Submission **Disable** (Step 2)
- Settings > Code security > Dependabot alerts/security updates **Enable** (Step 3 — dependabot.yml 활성)
- Settings > Branches > main > Branch protection rules `submit-pypi` required check 등록 여부 검증 (Step 1)

**잔여 영역 (사이클 87+ 점진 default)**:
- pylint 9.94 → 10.00 (잔여 36건 — C0415/C0301/W0718/W0613/R0913/R0917)
- D3 모바일 Phase 2 (Progressive Disclosure) — 사용자 명시 신호 의무 (사이클 81 보류)
- D1 i18n Phase 6 (DB persist + pybabel) — 사용자 명시 신호 의무 (사이클 84 보류)
- C2 Anthropic 200줄 추가 단계 (3.4배 → 1배) — 사용자 명시 사전 확인 의무 (사이클 85 보류)
- B1 i18n fallback baseline (2026-05-12 시간 의존)
...
### 사이클 83 — Tier B 11건 단일 응집 정책 진화 묶음 (2026-05-05 · 사이클 78~82 회고 후속 — 사용자 옵션 🅐 명시 결정)

사이클 78~82 회고 (#277) Tier B 11건 사용자 결정 회신 7건 (Q1~Q7) 후 옵션 🅐 (단일 응집 PR — 사이클 75 #250 패턴) 명시 진행. 정책 본문 진화 6건 + 메모리 영역 진화 3건 + 30초 체크리스트 강화 1건 = **응집 단위 단일 PR**.

| PR # | 제목 | 핵심 |
|------|------|------|
| **본 PR** Docs/cycle-83-tier-b-policy-evolution-bundle | **Tier B 11건 단일 응집 진화 묶음** — Q1 🅑 정책 9 완화 + Q3 정책 8 진화 + Q7 메모리 secondary cross-reference + Tier B-2/3/4/5/6 정책 3/1/5 진화 + 메모리 user-trust 진화 + 30초 체크리스트 강화 + STATE/CLAUDE 사이클 83 row |

**사이클 83 신규 LOC 합**: ~120 LOC docs/정책 본문 + ~30 LOC 메모리 = **~150 LOC** (정책 진화 응집 단위 — 1500 LOC 임계 미달 = 사용자 사전 확인 의무 X)

**자율 판단 보고 (정책 3 강화 — ⚠️ 마커 적용)**:
- (1) Tier B 11건 단일 응집 PR = 사용자 옵션 🅐 명시 결정 (사이클 75 #250 default 패턴 정합)
- (2) ⚠️ **Tier B-7 보류 사유** (`feedback-user-autofix-validation-pattern.md` 신설) = 사용처 1건 = 정책 16 4번 원칙 (≥ 3 임계) 미달 — 사이클 84+ 사용처 ≥ 3 도달 시 신설 결정
- (3) ⚠️ **Tier B-9 보류 사유** (메모리 deprecated 자동 분류 default) = 사이클 90+ 메모리 ≥ 40건 도달 시 — 현재 27건 = 임계 미달
- (4) ⚠️ **Tier B-10 보류 사유** (PAT 발급 재검토) = 사용자 Q4 🅑 명시 결정 (현행 유지) — 다음 5 사이클 누적 후 재검토 default
- (5) 회고 #277 §2-2 Tier B-11 (정책 8 진화 — 단일 작업일 ≥ 5회 사전 확인) 본 PR 적용 = ⚠️ 사이클 83 진입 자체가 본 진화 default 적용 첫 사례 (본 PR 만 진행 = ≥ 5회 영역 미진입)
- (6) 정책 9 완화 default 적용 첫 사례 = 본 PR 진입 자체가 검증 (회신 7건 받은 후 즉시 진행 = 정책 9 본문 위반 0)

---

### 사이클 82 — Tier B 묶음 + NEW-P0-1 머지 + 5+1 종결 회고 (2026-05-05 · 사이클 78~82 5 사이클 종결)
...
| 영역 | 처리 |
|------|------|
| **정책 15 신설** | 코드 작업 (add/edit/delete) 전 사전 사고 의무 + 이해 부족 시 중단/질문 의무. 3 자문 default ((a) 목적 정합 / (b) 영향 범위 인지 / (c) 검증 방법 명확) + 위임 분류 3-tier 통합 (High/Medium/Low) |
| **정책 16 신설** | 코드 단순화 default + 가독성 우선 (정확성/성능 유지). 4 원칙 우선순위 (정확성→성능→가독성→최소 추상화) + default 적용 패턴 (R0911~R0917 임계 default) + 금지 패턴 (사용처 ≥ 3 미달 추상화 금지) + 4 리뷰 체크 포인트 |
| **메모리 페어 2건** | `feedback-think-before-code-edit.md` (정책 15 페어) + `feedback-code-simplicity-default.md` (정책 16 페어). MEMORY 인덱스 13→15 (활성). 30초 체크리스트 grep 의무 default |
| **CLAUDE.md 갱신** | 정책 15/16 본문 (정책 14 직후) + tail 사이클 70 진입 + 30초 체크리스트 메모리 카운트 11→13 |
| **별도 PR 보류 (사이클 69 잔여)** | 정책 본문 카테고리 분류 (5 카테고리) — 사용자 결정 의무 (High tier — 사이클 70+ 결정 회신 대기) |

---

### 사이클 69 — 5+1 깊은 정합성 cleanup (2026-05-04 · #235 머지)

사용자 발화: *"현재까지 작업한 전체코드와 전체문서를 여러에이전트가 깊게 생각하여 문서 내용 및 파일 정리 작업을 수행을 해주세요"*

5+1 에이전트 (1차 5 병렬 + 2차 1 cross-verify general-purpose) 디스패치 결과:
...
| **별도 PR 권장** | 정책 본문 카테고리 분류 (5 카테고리 — 협업/PR운영/검증/MCP/관측) — 응집 단위 다름 + High tier 사용자 결정 의무 |
| **false-positive 차단** | 정책 14 PR 본문 형식 (#10) — L780 명시 정합 검증 |
...
**5-에이전트 병렬 회고 결과** (관점 5):
1. PR 분할 ROI: 5 PR → 권장 3 PR 분할 (응집 단위)
2. TDD 적용 효과: 누적 회귀 가드 모듈 + 사용자 패턴 (`importlib + AttributeError`) default 화
3. 자율 판단 정책 효과: 정책 1/3 = C 등급 (검토 환류 0) → 진화
4. 디자인 시스템 부분 적용: Stripe-style sparkline 누락 + Crimson Pro fallback 순서 역전
5. 사용자 ↔ Claude 동기화: 검증 항목 30+ 응답 0 → 정책 2 진화

**Claude 자유 발언 → 사용자 응답 반영**:
- 권장 패턴 검토 부담 우려 → 사용자 응답 *"실제 권장안이 좋은 내용"* → 정책 1 등급 C → A 정정
- PR 본문 검증 디테일 없음 → 사용자 응답 *"디테일한 검증은 없었습니다"* → **정책 11 신설**
- Railway 운영 신뢰 → 사용자 응답 *"Railway CI 검증으로 실패 시 빌드 차단"* → 운영 사고 0건의 신뢰도 ↑
- 정책 진화 + Phase 2 의무 → *"전부 승인합니다"*
...
**Phase 2 진입 전 사전 의무 (사용자 승인)**:
- A: 본 PR (정책 + 회고)
- B: pre-existing 7 fail 분류 PR
- C: `analysis_feedbacks` row 수 ≥10 검증
- D: Phase 2 착수 (Auto-merge 성공률 + AI 정합도 카드)
...
**사용자 조치 (사이클 85 종결 — Sentry 통합 폐기)**: Sentry 통합은 사이클 85 PR #317 로 완전 폐기되었다. Claude API 메트릭 + 파이프라인 타이밍 + MergeAttempt 관측은 Railway 로그 + DB 영구 기록으로 보존 (외부 SaaS 의존성 0). 폐기 runbook archive: [`docs/runbooks/_archive/sentry-activation.md`](runbooks/_archive/sentry-activation.md) (역사 자산).
