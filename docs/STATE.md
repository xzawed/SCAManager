...
**종합 수치**: 전체 **8146** 수집 (단위 **7939** + 통합 207) / E2E **124** (`#1291` 기준) / pylint **9.99/10** (`src/`).

| 지표 | 값 | 재측정 |
|---|---|---|
| 전체 테스트 | **8146 수집** | `py -3 -m pytest --collect-only -q tests/unit` + `tests/integration` — 단위 7939 + 통합 207 (현재) |
| E2E 테스트 | **124개** | `make test-e2e` — = 124 collected (113 표준 + 11 perf). 대조 `scripts/check_e2e_scope.py` |
...
- **현재** (7242→**7939** 단위; 통합 207 = **8146** 수집)
