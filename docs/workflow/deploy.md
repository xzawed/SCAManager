...
3. **기동** — `uvicorn src.main:app --host 0.0.0.0 --port $PORT --proxy-headers`(`railway.toml::startCommand`). import 시점에 `build_settings()`(`src/config.py::settings = build_settings()`) 가 돌아 설정 검증 실패면 기동이 막힌다.
...
1. `src/config.py` `Settings`(`src/config.py::class Settings(`)에 필드 선언. 기본값 없는 필드는 필수가 된다(현재 필수 3종 = `database_url` · `telegram_bot_token` · `telegram_chat_id`).
...
`is_production`(`src/config.py::def is_production`) = `ENVIRONMENT=production` 이거나 `APP_BASE_URL` 이 `https` 로 시작. 켜지면 `/docs`·`/redoc` 비노출 + 기본 `SESSION_SECRET` 기동 차단 + 스케줄러 기동이 함께 걸린다. 둘 다 없으면 공개된 기본 시크릿으로 뜬다.
