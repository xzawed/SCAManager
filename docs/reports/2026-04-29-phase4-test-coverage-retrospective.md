...
### `src/webhook/providers/github.py:279` head_commit=None NPE
...
`_extract_commit_message` (pipeline.py) 는 `data.get("head_commit")` 후 `if head:` 체크로 동일 패턴을 안전하게 처리. 두 군데 일관성 확보 필요.
