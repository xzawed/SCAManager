...
- [문서를 룰에서 프로세스로 — 3층 분리 + 5+1 회고 (2026-08-13~14 세션19, 8 PR #1340~#1347)](#문서를-룰에서-프로세스로--3층-분리--51-회고-2026-08-1314-세션19-8-pr-13401347)
...
- [STATE.md 이관 — 작업 이력 (2026-05-14 ~ 2026-06-12)](#statemd-이관--작업-이력-2026-05-14--2026-06-12)
...
## 문서를 룰에서 프로세스로 — 3층 분리 + 5+1 회고 (2026-08-13~14 세션19, 8 PR #1340~#1347)
...
**3층 분리** — 프로세스(`docs/process/` 5) · 함정(`.claude/traps.md`, 계열 A~E) ·
...
**단위 7143 → 7148** (통합 171 불변 = 7319 수집).

**5+1 회고**(`wf_2b615d5e-8c5` · **179 에이전트** · 14.2M 토큰 · verdict_coverage **1.0**)가
확정 143건(P0 6 · P1 53 · P2 84)을 냈고, **P0 는 둘 다 이 세션이 만든 것**이었다:
(a) 🔴 예산제의 분모가 4 glob 뿐이라 *"100% 달성"* 을 선언한 커밋이 **계수 밖에 마커 30개를
함께 만들었다** — 가장 값싼 우회로가 기호 치환이 아니라 **경로 이동**이었다
(b) 회고 범위가 손입력이라 `#1347` 이 회고를 통째로 피했다. 둘 다 같은 세션에서 고쳤다 —
분모를 6 표면으로 넓히고, 회고 워크플로에 **종료 시 범위 재확인**을 배선했다
(진입 시점 신탁은 *실행 중* 머지를 원리적으로 못 본다는 것이 이번의 진짜 기전이었다).

🔴 **세션 중 돌린 검증 2종이 P0-A 를 둘 다 놓쳤다** — Grok `019ffb93` 도 워크플로
`wf_d4c837bb-603` 도 *"이 가드가 이 규칙을 집행하는가"* 를 물었고, 아무도
*"이 가드의 **분모가 어디까지인가**"* 를 묻지 않았다.
...
## STATE.md 이관 — 작업 이력 (2026-05-14 ~ 2026-06-12)

> 2026-08-15 이관. `docs/STATE.md` §작업 이력 본문 전량(38,450자 · 46항목).
> 접은 이력과 제목이 겹쳐 보이나 본문은 유일본이라 한 줄도 접지 않았다.
> STATE.md 에는 헤딩+포인터만 남겼다 — `:7` 의 기존 단일 출처 계약이 이 이관을 승인한다.
>
> 🔴 아카이브 `STATE.md:N`(N≥128) 인용은 이 이관으로 **−32줄** 밀린다.
> `docs/_archive/**` 는 시점 기록이라 고치지 않았다. 오프셋 정본은 STATE.md §작업 이력 stub.
