...
- **사이클 115 (2026-05-21 · #555~#558)** — 사이클 114 5+1 회고 P1 3건 + 신규 발견 1건 구현. **#555 (A.1)**: `tests/unit/github_client/test_checks.py` `_classify_check_runs()` `waiting`/`pending`/`requested` 3상태 parametrized 회귀 가드 6케이스 신규 (test-writer 에이전트). 단위 2932→2938. **#556 (A.2/A.3/C.1/C.2)**: `src/static/css/themes.css` 4 테마 `--bg-2: var(--bg-card)` alias 추가 (팬텀 토큰 수정) + `tests/unit/webhook/test_telegram_provider.py` 3 semi-auto gate 콜백 테스트 `mock_save.assert_called_once()` assertion 추가 (GateDecision 영속성 회귀 가드) + `.claude/rules/pipeline.md` `asyncio.gather` Session 공유 금지 → `api.md` 교차 참조 + `docs/runbooks/workflow.md` 전체 읽기 원칙 → CLAUDE.md 정책 6 교차 참조. **#557** STATE.md/cycle-history/README 사이클 115 동기화. **#558 (신규 발견)**: `repo_insight_service.py` `repo_category_breakdown()` severity `.lower() in ("error","high")` → `.upper() in ("HIGH","ERROR")` 프로젝트 표준 통일 + `test_counts_by_category_and_severity` 대문자 `"HIGH"` 케이스 추가 (회귀 가드 강화).
