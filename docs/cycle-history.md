...
- **사이클 115 (2026-05-21 · #555 + #556)** — 사이클 114 5+1 회고 P1 3건 구현. **#555 (A.1)**: `tests/unit/github_client/test_checks.py` `_classify_check_runs()` `waiting`/`pending`/`requested` 3상태 parametrized 회귀 가드 6케이스 신규 (test-writer 에이전트). 단위 2932→2938. **#556 (A.2/A.3/C.1/C.2)**: `src/static/css/themes.css` 4 테마 `--bg-2: var(--bg-card)` alias 추가 (팬텀 토큰 수정) + `tests/unit/webhook/test_telegram_provider.py` 3 semi-auto gate 콜백 테스트 `mock_save.assert_called_once()` assertion 추가 (GateDecision 영속성 회귀 가드) + `.claude/rules/pipeline.md` `asyncio.gather` Session 공유 금지 → `api.md` 교차 참조 + `docs/runbooks/workflow.md` 전체 읽기 원칙 → CLAUDE.md 정책 6 교차 참조.
