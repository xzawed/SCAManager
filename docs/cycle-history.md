...
- [잔여/후속 세션 — U2 effects.js hx-boost 애니메이션 재초기화 (named init + document._fxEffectsHandler remove-before-add + seen WeakSet 멱등 가드, 단위 +1·E2E +1, 2026-06-13)](#잔여후속-세션--u2-effectsjs-hx-boost-애니메이션-재초기화-2026-06-13)
...
- **U2 effects.js 재초기화**: `effects.js` IIFE 가 `DOMContentLoaded` 에만 `init()` 바인딩 → hx-boost body swap 후 score-bar/SVG draw/count-up/entry 애니메이션 미재실행(opacity:0/0% 고착). **수정**: IIFE 유지하되 `init` 을 `document._fxEffectsHandler` 단일 슬롯에 저장 + `htmx:afterSettle`/`htmx:historyRestore` 에 remove-before-add 로 등록(ui.md PR #473 패턴) + 부분 swap 재애니메이션 방지를 위한 `seen` WeakSet 멱등 가드(`freshOnly` 헬퍼 — 새 DOM 노드만 처리, 잔존 노드 건너뜀).
- **TDD**: 정적 가드 `test_hx_boost_listener_guards.test_effects_animations_reinit_on_hx_boost`(remove-before-add 소스 단언, 단위 +1) + E2E `test_navigation.test_effects_init_reruns_on_hx_boost`(3회 hx-boost 재방문 후 `typeof document._fxEffectsHandler === "function"` + `body.fx-ready`, E2E +1). RED 양쪽 실패 확인 → GREEN.
- **수치**: 단위 4916→**4917** · E2E 113→**114** · 전체 수집 5070→**5071**. pylint 영향 없음(static JS). architecture.md 무변경(신규 파일 없음).
