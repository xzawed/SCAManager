...
🔴 **`block` 은 더 이상 편집을 막지 않는다** (2026-08-16 사용자 지시).
게이트는 세 판정을 **비판으로 전달**하고 편집은 진행된다 — LLM 판정은 기계로 검증할 수 없고,
막았을 때 저자가 우회하는 것이 관측됐기 때문이다(5회 전건 우회).
그래서 `block` 의 의미는 *"이 편집을 세우라"* 가 아니라 **"이것이 가장 심각한 지적이다"** 다.
등급은 여전히 중요하다 — 저자가 무엇을 먼저 볼지 정한다. **과잉 `block` 은 그 신호를 망친다.**
