...
### conftest.py 패턴
```python
# 🔴 환경변수는 src 임포트 전 직접 대입 (setdefault 금지 — 사이클 65 fix).
# setdefault 는 셸 env export 시 무시돼 운영 토큰이 settings 로 유입 + SESSION_SECRET(32자)
# 누락 시 config field_validator ValidationError 로 `from src.main import app` import 실패.
# 아래는 tests/conftest.py 실제 정본 부분집합 — 전체는 conftest.py 참조.
# Direct assignment (NOT setdefault) so test env always wins; SESSION_SECRET must be >=32 chars.
import os
...
os.environ["SESSION_SECRET"] = "test-session-secret-32-chars-long!"  # >=32자 필수
```
