...
const changedFiles = args?.changedFiles ?? null
...
다음 3회 호출을 실행한다 (Workflow 도구):
1. `Workflow({ name: 'integrity-audit', args: { scope: 'full', dryRun: true } })`
   기대 반환: `domains` 8개 전부 `['pipeline','gate','security','api','db','ui','docs','tests']`
2. `Workflow({ name: 'integrity-audit', args: { scope: 'area=gate', dryRun: true } })`
   기대 반환: `domains` = `['gate','pipeline']` (primary + adjacency)
3. `Workflow({ name: 'integrity-audit', args: { scope: 'diff', dryRun: true, changedFiles: ['src/gate/engine.py','docs/STATE.md'] } })`
   기대 반환: `domains` = `['gate','docs']`

> 셋 다 에이전트 0개 — 토큰 ~0. 이름 해소 실패 시(`.mjs` 미인식) 파일명을 `.js` 로 바꿔 재시도하고, 동작하는 확장자를 이후 Task 에 반영.
...
`Workflow({ name: 'integrity-audit', args: { scope: 'area=gate' } })` 실행.
...
2. `Workflow({ name: 'integrity-audit', args: {...} })` 호출.
...
`git diff --name-only main...HEAD` 로 현재 브랜치 변경 파일 수집 → `Workflow({ name: 'integrity-audit', args: { scope: 'diff', changedFiles: [...] } })`.
...
`Workflow({ name: 'integrity-audit', args: { scope: 'full' } })` 실행.
...
| 스크립트 확장자 | `.mjs` 우선, 이름 해소 실패 시 `.js` 폴백 (Task 1 Step 2) |
