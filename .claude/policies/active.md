...
**Why**: 회고 유예가 정책 5 종료 신호로 추적되지 않으면 무한 이월된다 (4연속 실증). 카덴스 트리거로 갭을 구조적으로 봉인 — 회고에서만 잡히는 프로세스 결함(자초 CodeQL·부분수정)의 실시간 재발 비용 차단. **How to apply**: 매 세션/사이클 종료(정책 5 종료 신호) 진입 시 "직전 정식 회고 이후 세션/PR 카운트" 1줄 자가 점검 → 트리거 충족 시 5+1 회고 강제 진입. 🔴 2026-08-05 정정: 이 지시는 원래 `정책 18 §4 3조건` 을 함께 가리켰으나 **정책 18 은 이 문서 자신이 폐기로 선언**했다(위 §정책 18 폐기 — 2026-07-10 Codex 구독 해지). 폐기된 정책을 조건으로 가리키는 살아있는 지시였으므로 제거했다. 카덴스 판정의 기계 근거는 `scripts/check_retro_cadence.py`(SessionStart) 다.
...
- 🔴 **A2-b (반례 일반화 + 봉인 선언 금지)** — **2026-08-13 사용자 결정 (A+B 혼합, backlog R83)**:
  Grok 이 준 반례를 red 로 만드는 것으로 끝내지 않는다. **(A)** 그 반례의 **클래스를 명명**하고
  클래스 내 **다른 인스턴스를 스스로 만들어** red 를 확인해야 *"닫았다"* 를 말할 수 있다.
  **(B)** 뮤테이션 red 는 *"이 뮤테이션을 막았다"* 로만 보고하고, *"fail-open 을 닫았다"* 는
  **외부 적대 검증 1회를 더** 거친 뒤에만 쓴다.
  집행: `tests/unit/scripts/test_agents_md_invariant_survival.py` — ⚠️ **어휘 생존만** 고정한다.
  어휘를 남긴 채 의미를 뒤집으면 통과한다(적대 검증 `wf_d4c837bb-603` 이 뮤테이션 3종으로 실증).
  이 규율의 실질 집행면은 review-time claim-review 다 — 그 한계를 지우지 말 것. 근거: 2026-08-12~13 한 세션에 2회 재발
  (`#1335` 아카이브 가드 · `#1340` 초판) — A 는 결함을, B 는 거짓 보고를 막는다.
  🔴 **기계 집행을 주장하지 않는다**(A 는 정적 판정 불가) — 어휘 생존만
  `tests/unit/scripts/test_agents_md_invariant_survival.py` 가 고정한다.
  본문 = [`AGENTS.md`](../../AGENTS.md) §불변식 2-b.
