...
completeness critic + **cross-verify=finding 강제**(모든 finding 이 verdict 수신, 단일 패스에서 일부만 검증되던 한계 해소).
...
1. 🔴 **범위는 기계에서 얻는다 — 손으로 적지 않는다** (정책 8: *"기계 산출 `scripts/retro_scope.py`"*):
   ...
   🔴 **디스패치 직전에 실행할 것.** 세션 초의 수치는 그 시점의 값이라 그 사이 머지분이
   빠진다. `git log` 로 손 조립하지 않는다.
...
> 🔴 `verdict_coverage < 1.0` = 일부 finding 이 verdict 미수신(UNVERIFIED). 단일 패스에서
> 일부만 검증되던 한계의 재발 신호 — 재실행 또는 수동 확인 권고 (리포트에 unverified 별도 표 노출).

## 정책 8 / 거버넌스
- 5+1 = **내부 self-verify**(관점 다양성). `pipeline-reviewer`/opus whole-branch 적대 리뷰와 **2-layer 독립** — 한쪽으로 다른 쪽 생략 금지 (구 외부 LLM mutual = 정책 18 폐기).
...
> 연계: `.claude/workflows/retrospective.mjs` 가 실행 엔진, 본 스킬이 진입점. loop-until-dry 정본 =
