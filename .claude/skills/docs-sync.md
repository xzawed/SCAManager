...
## 갱신 지점 (counts 변경 시 1~3·5 = 동일 값, check_docs_sync 4 지점 정합 의무)
1. **STATE.md 종합 수치** (header): `전체 **N** 수집 (단위 **M** + 통합 154)`
2. **STATE.md 추적셀 시작 헤더**: `**N 수집**` + `단위 M + 통합 154 (현재)`
3. **STATE.md 추적셀 trail**: 말미에 `+ **<날짜> <작업> +Δ** (...상세...). (\`pytest --co\` 단위 M + 통합 154 = N 수집).` 추가
4. **STATE.md 최신 블록**: 새 작업으로 **교체** (직전 서사는 cycle-history 최신순 맨 앞으로 이관 — 헤더 "직전" 체인 누적 금지)
5. **README.md + README.ko.md 배지**: `Tests-N%2B_total_(M_unit_%2B_154_integration)` (**양쪽 동일**·`%2B` 인코딩 유지)
6. **cycle-history.md**: TOC 엔트리(앵커 = 헤더 slug) + body 섹션(최신순 맨 앞)
