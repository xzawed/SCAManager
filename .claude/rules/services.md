...
- **2nd-LLM verifier (`src/verifier/`)**: `merge_verifier.verifier_blocks_merge`(should_verify + verify_merge_safety + 차단 시 PR 코멘트)는 `gate/engine._run_auto_merge` **단일출처**(자동·반자동 공유). `OPENAI_API_KEY` 미설정=완전 비활성(opt-in·비용 0). 활성화: [`docs/runbooks/merge-verifier.md`](../../docs/runbooks/merge-verifier.md). cross-ref [`api.md`](api.md) §2nd-LLM 검증자 가드.
