...
paths:
  ...
  - "src/mcp/**"
  - "src/cli/**"
  # `src/shared/**` — `claude_metrics.py` 가 background 세션 라우팅 규칙을 소비한다.
  - "src/shared/**"
---
...
- **배치 루프에서 소유권 토큰은 `claim_batch` 반환 직후 **값으로** 스냅샷한다**: `sessionmaker` 기본 `expire_on_commit=True` 라 첫 행 write-back commit 이 세션을 만료시킨다. 루프 안 `row.claim_token` 은 rows 2..N 에서 DB 재조회이고, 그 사이 다른 워커가 stale(>300s) 재클레임하면 **남의 토큰**을 자기 것으로 본다. 결과: 남의 claim 해제 → 정당한 소유자 `mark_terminal` CAS 0행 → 종결 소실 + 행이 `pending` 으로 부활. 현재 코드: `src/services/merge_retry_service.py:97` (`claim_tokens = {r.id: r.claim_token for r in claimed}`).
  - **계약**: 그 dict 를 루프 **진입 전**에 만들고, 모든 CAS write-back 에 전달 (`_process_single_retry(claim_token=...)` 포함).
  - 🔴 **회귀 가드는 `SimpleNamespace` · 단일 행 · `MagicMock` db 로 쓰지 않는다** — ORM 만료 / commit 경계 / identity map 이 없어 이 축을 못 본다. **실 ORM · 2행 이상 · 교차 세션 · 파일 SQLite** + `process_pending_retries` 실제 호출 (`tests/unit/services/test_merge_retry_batch_token_snapshot.py`). 시나리오만 재현하고 서비스 루프를 안 태우면 서비스 코드를 되돌려도 green.
  - **일반화**: commit 경계를 넘는 값은 ORM 속성이 아니라 지역 변수. `row.id`(PK)는 불변, 가변 필드는 재조회 대상.
  - **루프의 broad `except Exception` 은 시그니처 불일치도 삼킨다**: `_process_single_retry` 에 kwarg 를 추가하면 더 좁은 시그니처의 test fake 가 `TypeError` → `_recover_and_release` → "행이 pending" 으로 보인다. 신규 kwarg 시 `grep -rn "_process_single_retry" tests/` 로 test double 전수 (정책 16 · [[feedback-fixture-model-sync-discipline]]).
...
- **`issue_registration_service`**: GitHub Issue 를 **먼저 생성**한 뒤 DB INSERT — `IntegrityError`(중복/TOCTOU)=`DUPLICATE:N` / **비-IntegrityError(`SQLAlchemyError`)=orphan ERROR 로깅(repo/number/url/key)+재전파**. TTL 동기화 `_sync_state_if_stale` 예외는 `(httpx.HTTPError, KeyError, ValueError)` 흡수 — GitHub malformed 의 `resp.json()["state"]` 가 `GET /api/issues/*` 500 으로 나가지 않게.
- **`railway_client`**: GitHub payload None-able 키는 `(data.get(k) or {})` 정규화 + 토큰 인증 통과 후 비-dict JSON 바디는 `if not isinstance(body, dict): body = {}` (telegram provider 와 같음).
...
- **2nd-LLM verifier (`src/verifier/`)**: `merge_verifier.verifier_blocks_merge`(should_verify + verify_merge_safety + 차단 시 PR 코멘트)는 `gate/engine._run_auto_merge` **단일출처**(자동·반자동 공유). `OPENAI_API_KEY` 미설정=완전 비활성(opt-in·비용 0). 활성화: [`docs/runbooks/merge-verifier.md`](../../docs/runbooks/merge-verifier.md). cross-ref [`api.md`](api.md) §게이트 경로 단일화(:57 — 검증자 단일출처 불릿은 :63).
