...
- **SESSION_SECRET 2-layer 프로덕션 가드 (사이클 142 Phase B S2 #674)**: (1) `config.py` `field_validator` — 32자 미만 커스텀값 설정 시 `ValidationError` 앱 기동 차단 (임포트 시점). (2) `lifespan` startup — `APP_BASE_URL`이 `https://`로 시작하고 SESSION_SECRET이 기본값(`"dev-secret-key"`)이면 `RuntimeError` 기동 차단. 두 레이어 독립 보호 — `tests/unit/test_main.py::test_lifespan_raises_when_default_session_secret_in_prod` 회귀 가드.
