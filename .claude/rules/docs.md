...
이 절이 말하는 것은 **아카이브 파일을 재작성하지 말라는 규율**이지, 아카이브 내용이
보존됐다는 기계 단언이 아니다. `test_rules_archive_backlink.py` 는 역링크·앵커
도달성만 잰다. 서사가 공동화돼도 통과한다. 2026-08-15 사용자 결정(R81 옵션 b)이
보존 축을 내린 이유: 독립 재현 2회(2026-08-13 회고 · Grok `01a00061`)가
약 60% 공동화 아카이브에서 전건 green 을 보였다. `#1335` 본문의 보존 단언은
머지된 PR 이라 고칠 수 없고, `docs/_archive/rules-incident-log.md` 헤더의
"원문 보존" 도 시점 기록이라 그대로 둔다. 이후 *"아카이브가 보존한다"* 를 쓰는
사람은 집행되지 않는 주장을 하는 것이다.
