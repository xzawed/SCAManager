...
- **표 셀에 다시 적지 말 것** — 그 자리에 두면 머리와 꼬리가 한 줄에서 수만 자 떨어진다.
...
`docs/STATE.md` §테스트 수 추적 이력 현재 불릿 한 줄 + `check_docs_sync.py --fix` 가 남는다.
