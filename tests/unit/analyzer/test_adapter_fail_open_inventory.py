...
로 좁힌 어댑터만 갖는다. **몇 개인지는 여기 적지 않는다** — 어댑터가 늘거나 전환될 때마다
늙는다(실측: 이 자리의 「10/23」은 23이 `dotnet_format` 삭제 이전 수라 두 번 틀렸다).
어느 것이 남았는지도 이 파일이 아니라 **탐지기**가 말한다 — 아래 목록은 그 출력과 대조될
뿐이고, 목록을 손으로 읽어 판단하지 않는다.
