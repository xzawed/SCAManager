...
여기서 `if True:` 는 도구 출력을 전혀 보지 않는다. 살아 있는 `raise` 는 `JSONDecodeError`
핸들러의 것뿐이라 잡는 것은 「파싱이 예외를 냈다」뿐이고, 크래시 stdout 이 깨끗한 것과
바이트가 같으면 여전히 «이슈 0건 · 완전» 이다.
...
class _Analyzer:
    ...
    def run(self, ctx) -> list:
        if True:  # noqa: SIM108 — 🔴 도구 출력을 보지 않는다. 판별식이 아니다.
            try:
                ...
                return []
        return None  # 🔴 축 B 를 켜지 않는다 — `return []` 이면 D 를 격리하지 못한다.
