...
class TestDecryptTokenFallback:
    ...
    def test_decrypt_does_not_raise_on_invalid_token(self, monkeypatch):
        ...
        _reset_fernet_cache()
        from src.crypto import decrypt_token
        # 🔴 try/except 로 감싸지 않는다 — 예외가 나면 pytest 가 그대로 traceback 으로 실패
        #   보고하므로("does_not_raise" 테스트의 본질), 감싸면 두 관측자를 동시에 깬다:
        #   (a) S5779 = except 가 AssertionError 를 삼킴 (b) CodeQL py/uninitialized-local
        #   = result 가 예외 경로에서 미할당. 직접 호출이 두 관측자 모두를 만족한다.
        # A raised exception already fails the test; wrapping it trips S5779 + py/uninitialized-local.
