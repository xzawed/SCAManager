...
add_repo.html 의 pagehide 리스너와 effects.js 리스너가 remove-before-add 패턴
(named handler + 선행 removeEventListener)을 유지하는지 소스 정적 스캔으로 회귀 차단.
hx-boost body swap 재실행 시 익명 리스너가 window/document 에 누적되던 사고(#35/#36)를 봉인한다.

Static source scan guard: ensures the pagehide (add_repo.html) and effects.js listeners
keep the remove-before-add pattern so they do not pile up on hx-boost body-swap re-execution.
"""
import pathlib
...
def test_oncein_view_has_io_miss_safety_net():
    ...
    # 🔴 one-shot 금지 — scroll/resize 지속 sweep 으로 below-fold 요소도 복구.
    ...
    # P1 regression guard: no blanket dispose may return to init() (it tears down the prior
    ...
    ), "init() 일괄 dispose 재유입 → P1 count-up '0' 고착 회귀 (이중 init + freshOnly 재등록 차단)"
